...
> **心跳 52 变更**：**D-5a 已按建议 A 落地并实机闭环**（不再是待拍板项）。详见下方「心跳 52 做了什么」。
...
| **T-49** 🆕 | **「改楼层」和「换变体（swipe）」在界面上根本点不到。** 设备侧全 DOM 枚举 **176** 个带 `title`/`aria-label` 的元素：「编辑」**0 命中**、「变体/swipe」**0 命中**；而「回退/重新生成」「世界书」「剧情控制台」「MVU 状态」「导入」**全部命中**。悬停消息节点后按钮数 **134 → 134 不变**（排除"hover 才显示"），也**没有任何右键菜单容器**；并已确认当前就是 RP 会话视图（RP 脚本 pill 挂在 composer 里）| **建议补**。理由：**引擎在、入口不在** —— `variant/groups` 数据面返回 `200 groups=1`、`th-edit`/`th-append` 写桥也已实现，即"能改能存能读，但用户点不到"；基准 TT 有这两项（Tavern 核心交互），按"跟基准一致不能少"属功能差异<br>**不自行补的原因**：入口放哪（用 DSH 原生消息操作栏 vs DSHT 自建操作栏）是方向性选择 | 用户**无法编辑已发出的楼层、无法切换/重生成变体**（只能整轮重新生成） |
