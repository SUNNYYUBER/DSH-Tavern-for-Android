...
> 最后更新：2026-09-11（心跳 55 —— **修掉「重新生成 / 回退」两条 live 写入路径恒 500（死了 8 个心跳）**）
...
| **二 · 和 TT 逐项对齐** | 截获双方发给 AI 的完整请求，逐条对比，修差异 | 🔄 **进行中**（8 项差异，5 项已闭环；**心跳 54：数据面 + UI 面的长尾回归（回退/编辑/变体/世界书/MVU）已逐项覆盖**，`stage4-regression` **21/21 全过**；**心跳 55：修掉「重新生成 / 回退」两条 live 写入路径恒 500 的缺陷（真机闭环）**；仅「变体切换」的端到端待构造 ≥2 变体） |
| **三 · 发布前准备** | 大扫除 / README / GitHub Releases + 更新开关 | ⏳ **进行中**（README ✅、更新开关骨架 ✅、大扫除未做） |
...
| **D-5b** | 要不要**一次性清洗存量脏楼层**（`$1` 残留 + `<interactive_input>` 包装被写回过） | **只读报告已产出**（心跳 54）→ **真脏 83 条 / 13 个会话**（`$1` 字面残留 62 / 嵌套包装 1），另有 35 条属**设计用途快照**不计风险；**修复已生效**（最后一次污染 09-10 08:53 UTC → 最新消息 09-11 08:12 UTC，**23.3 小时零新增**）。<br>**A** = 清洗（需写带 `.bak` + 幂等 + 收敛断言的迁移器，不可逆，先备份再副本验证）；**B** = 不清洗 | 老会话正文里留可见的 `<interactive_input>` / `$1` 残留（**只影响旧会话观感，与新版行为无关**） |
...
## 心跳 53 做了什么（**回头审「已修」标记本身 —— 7 项里 5 项标记不成立**）

> 一句话：这一轮没有去找新 bug，而是按「完成审计」的要求**回头核验已经打了 ✅ 的项**，
> 结果 §4 兼容面 7 项里 **5 项的 ✅ 与代码现状不符**。全部已真修 + 补反控。
...
**共同成因（沉淀 LEARNINGS L61）**：五条里四条是「同一语义存在多份副本，只改了其中一份」
（`engine.ts` / `display-compiler.ts` / `th-shim.ts` / `lib/*.js` 产物）；另两类是「注释声称了
机制上不可能的事」与「数据全为默认值掩盖了实现错误」（实测 2 份真实 settings：`substituteRegex`
分布 `{"0":9}` / `{"0":21}`，`trimStrings` 非空 0 条 —— 所以这两个字段的缺陷长期不暴露）。

**修与验**：
- 五条全部真修；新增/重写测试 **38 条**（含跨模块**差分断言**：shim 的 `Mvu.parseMessage`
  ≡ `state/mvu.ts` 的 `parseUpdateVariable`，把"两侧手工同步"的硬约束用断言锁住，而非靠注释声称）。
- **每条都做了反控**（stash 源码 → 用例必须转红）：T-16 5 红 / T-17 2 红 / T-18 5 红 /
  T-19 5 红 / T-22 5 红 / T-21 1 红 —— 全部成立。
- **推翻一条被固化的错期望**（L50/L58 第三例）：原测例「`{{match}}` 引用与 trimStrings」
  断言的正是缺陷行为，已按基准重写。
- 单测 **971 → 1022**；三闸门 `typecheck` **0 错**；产物已重建（含两条注入期转义错误修正）。
- 顺手核到的第六处：**T-21 的 `delete_occurred` 值补对了、路径口径没跟上** —— 判存函数
  只按 `.` 切分，而 host 用 **lodash 路径**（`a.list[0]`）→ 下标路径恒判「不存在」→
  `delete_occurred` **永远是 false**（脚本据此误判"没删掉"）。已镜像 host 分词并补 3 例断言。
  （T-20 / T-21 的其余字段有 `th-regex-contract.spec.ts` 与 `getChatMessages` 形状测试支撑，核验通过。）
