...
## 心跳 46 做了什么（阶段三推进；不改任何功能行为，全在修欠账）

> 一句话：**发现并修掉一个"上线三天从未生效"的功能**，并补上前两轮反复踩坑的那类防线的缺环。

| # | 结论 | 证据 |
|---|---|---|
| 1 | 🔴 **「我装的到底是不是最新包」这个功能**（2026-09-08 为你做的）**从未生效过** —— 前端用 POST 调，后端只挂在 GET 上 → 恒 404，被前端 `.catch` 吞成"没数据" | 实机：`GET`→200 / `POST`→404（对照 `POST /rp/status`→200）；源码 `MigrationStatusPanel.tsx:47` |
| 2 | 同型问题**全库只有这一处**，已建常备防线 | 新脚本 `scripts/audit-route-contract.mjs`：67 个前端路由 **0 违约**；用旧代码做负向对照能精确报出该违约 |
| 3 | ✅ **「版本与更新」面板上线**（T-27 不依赖外部决定的部分）：显示本机版本 / DSH 版本 / 构建标记，可配置更新源、一键检查更新、**按本机架构挑下载包** | 实机 8 条判据全过（含 GitHub 与静态 JSON 两种源形态、新旧/相同/更旧/看不懂/网络失败/未配置各分支）；截图存 `stage3-device/heartbeat46/` |
| 4 | 🔴 又抓到一个**静默缺陷**：`worldbook.json` 会被写成 `[object Object]` 的**乱码二进制**（写得成功、无报错、内容全错） | 类型检查暴露：上游漏 `JSON.stringify` + 下游"对象即二进制"容错过宽。已双向修 |
| 5 | 🟠 **`npm run typecheck` 此前形同虚设**（2546 个错误）→ 意味着「用类型拦住布尔/计数混比」这条防线**当时并不存在** | 已建 `npm run typecheck:core` → **核心逻辑层 0 错误**；**正控**注入 `boolean === 0` 确认报 TS2367 |
| 6 | 上面这条闸门一开就抓出 3 个真问题（含一个"任何调用方漏传参数就静默跳过整段 TT 投影"的隐患） | 见 `.goal/upgrade-0.1.5/LEARNINGS.md` L22–L25 |

**交付**：双架构 APK 重打（x86_64 debug + arm64 release），825+ 单测全绿。
