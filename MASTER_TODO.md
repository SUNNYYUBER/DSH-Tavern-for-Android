...
> 最后更新：2026-09-11（心跳 57 续 —— **阶段二长尾「变体切换端到端」闭环**：设备上**从未存在过**任何变体组，本轮走产品路由造出 2 成员组并完成切换，写入形态经官方 `Session` 构造器判定合法；另沉淀 L74/L75 两条经验，登记 T-57）
...
| # | 要你定的事 | 我的建议 | 不定会怎样 |
|---|---|---|---|
| ~~**T-42**~~ ✅ | ~~卡脚本要挂到 **ST 正则面板的 17 个 DOM 元素**（`#saved_regex_scripts` 等），我们前端是 DSH 的 UI，**一个都没有**~~ | **心跳 57 根因修正 → 已修，不用你拍板**：真正的根**不是 DOM，而是我方宿主页缺 ST 标准的 `/version` 端点**（实机 `GET /version → 404`）→ 卡的 `fetch('/version')` 落 `.catch(() => 10000)` → 脚本内 **20+ 处** `versionNumber >= 11305` 分叉**全部走旧版** → 才去找旧版正则面板 DOM。基准 TT 声明 `SILLYTAVERN_COMPAT_VERSION = '1.18.0'`（→ 11800）**走新版路径**。<br>**已按基准一组三件补齐**：`/version` + `chatCompletionSettings.extensions.regex_scripts`（真数据）+ `#saved_regex_scripts` 锚点。见 **T-56** | — |
...
- **T-57（待定性）**：`sessionController` 服务在应用运行一段时间后变为 unavailable，
  而 RP UI 的「打开角色会话」是**唯一**依赖 `session.list`/`session.create` 的产品路径
  （`RpOverlay.tsx:334/345`）⇒ 那种状态下该入口**必然失败**。触发条件待定性（重启即恢复）。
- **T-58（待定性）**：`/rp/session-regenerate` 与 `/rp/session-rollback` 的**非 live 分支是破坏性的**
  （物理截断会话文件）。建议加一道显式门槛（非 live + 未显式声明 `allowFileTruncation` 时拒收），
  需先确认产品路径是否会碰到它。
