...
| **二 · 和 TT 逐项对齐** | 截获双方发给 AI 的完整请求，逐条对比，修差异 | 🔄 **进行中**（8 项差异，5 项已闭环；心跳 51 起转为「先把卡脚本能跑通的宿主面补齐」—— 缺口 13 → 1） |
...
| **T-46 / T-47** 🆕 | **心跳 51 新登记的两个"不能靠接线解决"的缺口。**<br>**T-46**：`iframe 门面` 比 `宿主页门面` 窄得多（**9 个成员 vs 37 个成员**；卡脚本访问的 19 个成员里 **16 个在 iframe 面缺失**）。原因不是忘了写，而是 `th-shim.ts` 是**构建期注入的字符串**，不能 `import` 共享模块（与 T-41 同约束）。要不要把 iframe 面"本体化"（可行性三分类：可经 postMessage 桥 / 必须同步 / 需注入式装配）<br>**T-47**：剩余 3 个宿主缺口 `characters`(卡调用 4 次) / `characterId`(12 次) / `chatMetadata`(9 次)。**它们缺的不是接线而是数据模型**：`characters`+`characterId` 必须**成对**建真实角色列表模型（单给一个会互相矛盾）；`chatMetadata` **我们根本没有对应存储**，给个 `{}` 只会让写入**静默消失** | **T-46 建议做**（不然 16 个成员在卡里永远是 undefined，报错会落在卡内部、归因困难）。<br>**T-47 建议先定模型再实现**：角色列表可以映射到我方现有的"角色卡 + 世界书"数据；`chatMetadata` 需要先决定**存哪**（会话 sidecar / 插件 state），否则实现出来就是新的静默失败 | 卡脚本在 iframe 里访问这 16 个成员会拿到 `undefined`（不一定立刻报错，可能表现为"某个功能悄悄不生效"） |
