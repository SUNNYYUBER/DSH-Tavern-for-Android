...
export function normalizeSheet(raw: unknown, index = 0): Sheet | null {
  ...
  const headers = Array.isArray(r.headers)
    ...
    : []
  const rows: string[][] = Array.isArray(r.rows)
    ? r.rows.filter(row => Array.isArray(row)).map(row => (row as unknown[]).map(c => String(c ?? '')))
    : []
 ...
 */
export function migrateLegacyTables(raw: Record<string, unknown>): { sheets: Sheet[]; found: boolean } {
  ...
  const entries: Array<[string, unknown]> = Array.isArray(legacy)
    ? (legacy as unknown[]).map((v, i) => [`表${i + 1}`, v])
...
export function sheetsStatePath(dshHome: string, sessionId: string): string {
  return join(dshHome, 'rp', 'state', `${sessionId}.json`)
}
 ...
 */
export async function loadSheets(dshHome: string, sessionId: string): Promise<LoadedSheets> {
  if (!isValidTablesSessionId(sessionId)) return { whole: {}, sheets: [], history: [], migrated: false }
  let whole: Record<string, unknown> = {}
  try {
    const parsed = JSON.parse(await readFile(sheetsStatePath(dshHome, sessionId), 'utf8')) as unknown
    if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) whole = parsed as Record<string, unknown>
  } catch {
    return { whole: {}, sheets: [], history: [], migrated: false } // 无文件/坏 JSON = 空
  }
  // 归一 sheets（坏条目跳过）
  let sheets: Sheet[] = Array.isArray(whole.sheets)
    ? (whole.sheets as unknown[]).map((s, i) => normalizeSheet(s, i)).filter((s): s is Sheet => s !== null)
    : []
  let migrated = false
  ...
  const history: SheetHistoryEntry[] = Array.isArray(whole.sheetHistory)
    ? (whole.sheetHistory as unknown[]).map(h => {
        const e = (h ?? {}) as { at?: unknown; sheets?: unknown }
        return {
          at: typeof e.at === 'number' && Number.isFinite(e.at) ? e.at : 0,
          sheets: Array.isArray(e.sheets)
            ? (e.sheets as unknown[]).map((s, i) => normalizeSheet(s, i)).filter((x): x is Sheet => x !== null)
            : [],
        }
      })
    : []
...
export function pushSheetHistory(history: SheetHistoryEntry[], sheets: Sheet[], now = Date.now()): SheetHistoryEntry[] {
  const next = [...history, { at: now, sheets: cloneSheets(sheets) }]
  return next.length > SHEET_HISTORY_MAX ? next.slice(next.length - SHEET_HISTORY_MAX) : next
}

// ---------------------------------------------------------------------------
// E2：tableEdit 指令（解析 + 执行）
// ---------------------------------------------------------------------------
...
const OPS: TableEditOp[] = ['insertRow', 'updateRow', 'deleteRow', 'insertCol', 'deleteCol', 'setName', 'rename']
 ...
 */
export function parseTableEdits(text: string): { edits: TableEdit[]; skipped: number } {
  ...
  for (const m of text.matchAll(BLOCK_RE)) {
    ...
    for (const rawLine of rest.split('\n')) {
      ...
      if (op === 'insertRow') {
        ...
        if (restArgs.length < 1 || !isIntToken(restArgs[0])) { skipped++; continue }
        ...
        if (values.length === 0 || !values[0]) { skipped++; continue }
...
const FN_OPS: TableEditOp[] = ['insertRow', 'updateRow', 'deleteRow', 'insertCol', 'deleteCol']
...
function findCloseParen(src: string, openPos: number): number {
  let depth = 0
...
function splitFnArgs(s: string): string[] {
  ...
  for (const ch of s) {
    ...
    if (ch === '{' || ch === '｛') { braceDepth++; cur += ch; continue }
    if (ch === '}' || ch === '｝') { braceDepth = Math.max(0, braceDepth - 1); cur += ch; continue }
    if (braceDepth === 0 && (ch === ',' || ch === '，')) { out.push(cur); cur = ''; continue }
    cur += ch
  }
 ...
 */
function parseLooseDict(src: string): Record<string, string> | null {
  const t = src.trim()
...
function parseFnArgs(src: string): Array<number | string | Record<string, string>> {
...
function dictToPositional(data: Record<string, string>): string[] {
  const keys = Object.keys(data).filter(k => /^\d+$/.test(k)).map(Number).sort((a, b) => a - b)
  if (keys.length === 0) return []
  const out: string[] = []
  for (let i = 0; i <= keys[keys.length - 1]; i++) out.push(data[String(i)] ?? '')
  return out
}
 ...
 */
function fnCallToEdit(op: TableEditOp, args: Array<number | string | Record<string, string>>): TableEdit | null {
  if (args.length === 0) return null
  const a0 = args[0]
  if (typeof a0 !== 'number' || !Number.isInteger(a0) || a0 < 0) return null
  const sheetIndex = a0
  if (op === 'insertRow') {
    const data = args.find(a => typeof a === 'object') as Record<string, string> | undefined
    if (!data) return null
    return { sheet: '', sheetIndex, op, values: dictToPositional(data) }
  }
  if (op === 'updateRow') {
    const rowArg = args[1]
    const data = args.find((a, i) => i >= 2 && typeof a === 'object') as Record<string, string> | undefined
    if (typeof rowArg !== 'number' || !Number.isInteger(rowArg) || rowArg < 0 || !data) return null
    const values = Object.entries(data).map(([k, v]) => `${Number(k) + 1}=${v}`)
    return { sheet: '', sheetIndex, op, row: rowArg + 1, values }
  }
  if (op === 'deleteRow') {
    const rowArg = args[1]
    if (typeof rowArg !== 'number' || !Number.isInteger(rowArg) || rowArg < 0) return null
    return { sheet: '', sheetIndex, op, row: rowArg + 1, values: [] }
  }
  if (op === 'insertCol') {
    ...
    for (const a of args.slice(1)) {
      if (typeof a === 'number' && col === undefined) col = a + 1
      else if (typeof a === 'string' && name === undefined) name = a
    }
  ...
  // deleteCol
  const colArg = args[1]
  if (typeof colArg !== 'number' || !Number.isInteger(colArg) || colArg < 0) return null
 ...
 */
export function executeTableEdits(sheets: Sheet[], edits: TableEdit[]): { sheets: Sheet[]; applied: number; skipped: number } {
  ...
  for (const edit of edits) {
    ...
    if (edit.op === 'insertRow') {
      ...
      if (!Number.isInteger(pos) || pos < 1 || pos > sheet.rows.length + 1) { skipped++; continue }
      const cells = sheet.headers.map((_, i) => edit.values[i] ?? '')
      sheet.rows.splice(pos - 1, 0, cells)
      applied++
    } else if (edit.op === 'updateRow') {
      if (!Number.isInteger(edit.row) || (edit.row as number) < 1 || (edit.row as number) > sheet.rows.length) { skipped++; continue }
      const row = sheet.rows[(edit.row as number) - 1]
      let touched = 0
      for (const v of edit.values) {
        ...
        if (eq > 0) {
          ...
          if (isIntToken(key)) col = Number(key)
          else {
        ...
        if (col !== null) {
          if (col < 1 || col > sheet.headers.length) continue
          row[col - 1] = v.slice(eq + 1)
          touched++
        } else {
          const pos = touched + 1
          if (pos > sheet.headers.length) continue
          row[pos - 1] = v
          touched++
        }
      }
      ...
      if (!Number.isInteger(edit.row) || (edit.row as number) < 1 || (edit.row as number) > sheet.rows.length) { skipped++; continue }
      sheet.rows.splice((edit.row as number) - 1, 1)
      applied++
    } else if (edit.op === 'insertCol') {
      const pos = edit.col ?? sheet.headers.length + 1
      if (!Number.isInteger(pos) || pos < 1 || pos > sheet.headers.length + 1) { skipped++; continue }
      sheet.headers.splice(pos - 1, 0, edit.values[0] ?? '')
      for (const row of sheet.rows) row.splice(pos - 1, 0, '')
      applied++
    } else if (edit.op === 'deleteCol') {
      if (!Number.isInteger(edit.col) || (edit.col as number) < 1 || (edit.col as number) > sheet.headers.length) { skipped++; continue }
      if (sheet.headers.length <= 1) { skipped++; continue } // 至少保留 1 列
      sheet.headers.splice((edit.col as number) - 1, 1)
      for (const row of sheet.rows) row.splice((edit.col as number) - 1, 1)
      applied++
    } else {
...
export function renderTableData(sheets: Sheet[]): string {
 ...
 */
export function renderTablePrompt(sheets: Sheet[]): string {
...
export function getTableCell(sheets: Sheet[], name: string, row: number, col: number): string | null {
 ...
 */
export function parseA1Address(addr: string): { row: number; col: number } | null {
 ...
 */
export function expandTableMacros(text: string, sheets: Sheet[]): string {
  let out = text
  ...
  if (/\{\{\s*GET::/i.test(out)) {
    ...
    out = out.replace(/\{\{\s*GET::([^}]+)\}\}/gi, (_all, body: string) => {
