...
export type ProjKind =
  | 'agent-manual'   // DSH 自身 agent 说明书（保持原位，属基础设施）
  | 'user-input'     // 真实用户输入
  | 'history-user'   // 历史楼层里的用户发言
  | 'history-ai'     // 历史楼层里的 AI 发言
  | 'fold-marker'    // 影子化折叠标记（投影时丢弃）
  | 'runtime-ctx'    // DSH 运行时上下文快照
  | 'skill-list'     // skill 清单（DSH 基础设施）
  | 'system-level'   // 角色卡/世界书/记忆/状态树/预设/文风等 → 应进 system
  | 'unknown'
 ...
 */
export function classify(m: ProjMessage): ProjKind {
  const role = typeof m.role === 'string' ? m.role : ''
  const text = textOf(m.content)
  if (text.length === 0) return 'unknown'
  if (FOLD_MARKERS.some((k) => text.startsWith(k))) return 'fold-marker'
  // DSH 基础设施：agent 说明书必须留在最前（它是 system 语义，TT 无对应物）
  if (text.startsWith('You are an AI agent powered by DeepSeek Harness')) return 'agent-manual'
  if (text.includes('A skill is a reusable set of')) return 'skill-list'
  if (text.startsWith('<interactive_input>') || text.startsWith('<interactive_input>\n')) {
    // 运行时上下文快照也包在 interactive_input 里，需先区分
    if (text.includes('Current runtime context')) return 'runtime-ctx'
    return 'user-input'
  }
  if (text.includes('Current runtime context')) return 'runtime-ctx'
  if (role === 'assistant') return 'history-ai'
  // 系统级 RP 内容（DSHT 侧一律以 user 承载 —— 正是 D-3 要纠正的错位）
  if (
    text.includes('你正在进行角色扮演')        // 角色卡
  ...
  if (role === 'user') return 'history-user'
  return 'unknown'
}

// ---------------------------------------------------------------------------
// D-3：system 槽位路由（`system-prompt/assemble` 的唯一合法消费者）
// ---------------------------------------------------------------------------
 ...
 */
export function planSlotSections(
  batch: SlotBatch,
...
export const PROJ_DEFAULTS: ProjConfig = {
  enabled: true,
  systemSlot: true,
  userInputFirst: true,
 ...
 */
export function projectToTtShape(
  messages: readonly ProjMessage[],
  cfg: ProjConfig = PROJ_DEFAULTS,
): ProjResult {
  ...
  const userInputs: string[] = []    // → user（提前）
  const infra: Array<{ role: 'system' | 'user' | 'assistant'; content: string }> = []
  const timeline: Array<{ role: 'user' | 'assistant'; content: string }> = []
  ...
  if (!cfg.systemSlot) {
    const restored: Array<{ role: 'system' | 'user' | 'assistant'; content: string }> = []
    for (const m of messages) {
      if (classify(m) === 'fold-marker') continue
      restored.push({
  ...
  if (cfg.userInputFirst) {
    for (const u of userInputs) out.push({ role: 'user', content: u })
  }

  // 4) DSH 基础设施（skill 清单/运行时上下文）紧随其后
  for (const m of infra) out.push(m)

  // 5) 历史楼层
  for (const m of timeline) out.push(m)
