...
function findPreviewTarget(el: Element | null): { kind: 'mention' | 'link'; name: string; url: string | null } | null {
 ...
 */
function looksTextual(contentType: string, path: string): boolean {
...
function imageLike(path: string): boolean {
...
function openPreview(name: string, content: { kind: 'text'; value: string } | { kind: 'image'; url: string } | null): void {
  closePreview()
  ...
  root.append(panel)
  // 点遮罩关闭（点面板本身不关）
  root.addEventListener('click', e => { if (e.target === root) closePreview() })
...
export function installFilePreview(doc: Document): () => void {
  ...
  const onClick = (e: MouseEvent): void => {
    ...
    if (imageLike(path)) { openPreview(hit.name, { kind: 'image', url }); return }
    void (async () => {
      try {
