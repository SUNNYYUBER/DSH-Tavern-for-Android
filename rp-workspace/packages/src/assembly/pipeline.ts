...
import { runRegexScripts, type RegexScript, PLACEMENT } from '../regex/engine.ts'
import type { CompiledSlot } from '../preset/schema.ts'
import type { ActivatedEntry } from '../lore/trigger.ts'
import { WI_POSITION } from '../lore/entry.ts'
...
export interface AssemblyTrace {
  ...
  activatedEntries: Array<{ comment: string; reason: string; book: string }>
  unknownMacros: string[]
  tokenEstimate: { prompt: number; droppedMessages: number }
}
...
const defaultEstimate = (s: string) => Math.ceil(s.length / 4)
...
export function assemble(
  ...
  wiActivated: ActivatedEntry[],
  history: AssemblyMessage[],
  state: unknown,
  regexScripts: RegexScript[],
  macroCtx: Omit<MacroContext, 'getState'> & { getState: (path: string) => unknown },
  ...
  let processedHistory = history
    ...
    .map(m => {
      const placement = m.role === 'user' ? PLACEMENT.USER_INPUT : PLACEMENT.AI_OUTPUT
      const r = runRegexScripts(regexScripts, m.content, 'prompt', placement, { depth: null })
      for (const h of r.hits) trace.regexHits.push({ scriptName: h.scriptName, count: h.count })
      return { ...m, content: r.text }
    })

  // ---- 步骤 2：世界书内容条目（激活结果分桶：顶部/深度/示例区）----
  const evalCtx: MacroContext = { ...macroCtx }
  const evalText = (t: string) => {
  ...
  const wiDepthInjections: Array<{ depth: number; role: string; content: string }> = []

  for (const a of wiActivated) {
    // 世界书条目内容过正则（placement=WORLD_INFO，ST 语义）
    const r = runRegexScripts(regexScripts, a.entry.content, 'prompt', PLACEMENT.WORLD_INFO, { depth: null })
    for (const h of r.hits) trace.regexHits.push({ scriptName: h.scriptName, count: h.count })
  ...
  // ---- 步骤 3：槽位骨架落位 ----
  const markerContent = (id: string): string => {
    switch (id) {
      ...
      case 'stateSummary': {
        ...
        if (estimate(json) < 2000) return `<State>\n${json}\n</State>`
        const keys = typeof state === 'object' ? Object.keys(state as object) : []
  ...
  const headMessages: AssemblyMessage[] = []
  ...
  const messages: AssemblyMessage[] = [...headMessages, ...processedHistory]
  const roleRank = { system: 0, user: 1, assistant: 2 }
  const injectionsByDepth = new Map<number, Array<{ role: string; content: string }>>()
  ...
  for (const [depth, list] of [...injectionsByDepth.entries()].sort((a, b) => b[0] - a[0])) {
    list.sort((a, b) => roleRank[a.role as keyof typeof roleRank] - roleRank[b.role as keyof typeof roleRank])
...
function buildConfigSummary(slots: CompiledSlot[], wiCount: number): string {
