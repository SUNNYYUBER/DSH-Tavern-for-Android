...
function readVarPath(tree: Readonly<Record<string, unknown>>, path: string): unknown {
...
function timeParts(now: Date): { time: string; date: string; weekday: string } {
 ...
 */
export function expandDisplayMacros(text: string, ctx: DisplayMacroCtx | null): string {
  if (!text.includes('{{')) return text // 快路径：绝大多数正文无宏
  if (ctx === null) return text
  const now = new Date()
  const { time, date, weekday } = timeParts(now)
  const variables = ctx.variables ?? {}
  return text.replace(MACRO_RE, (raw, key: string) => {
    ...
    if (lower === 'weekday') return weekday
    ...
    if (colon > 0) {
      ...
      if (name === 'roll') {
        ...
        if (!Number.isInteger(max) || max < 1) return raw
        const rollOnce = (): number => 1 + Math.floor(Math.random() * max)
    ...
    if (customBare !== undefined) return customBare
    // 未知宏原样保留
    return raw
  })
}
