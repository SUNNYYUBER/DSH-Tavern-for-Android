...
async function postJson(url: string, body: unknown): Promise<{ ok: boolean; json: Record<string, unknown> }> {
  ...
  return { ok: resp.ok, json: await resp.json().catch(() => ({})) as Record<string, unknown> }
}

/** MVU 自检：专用 __selftest__ 会话写入 canary 变量再读回（写读闭环）
 * + 批次修复 8：状态栏实测（/dsht-mvu/statusbar 返回 html 即活） */
async function selftestMvu(): Promise<boolean> {
...
async function selftestPromptTemplate(): Promise<boolean> {
...
export const PROBES: PluginProbe[] = [
  {
    label: '变量 / 状态栏（MVU）',
  ...
  },
  {
    ...
    selftest: async () => {
      ...
      if (!r.ok) return false
      const j = await r.json().catch(() => ({})) as { cursor?: unknown; lastFloor?: unknown }
