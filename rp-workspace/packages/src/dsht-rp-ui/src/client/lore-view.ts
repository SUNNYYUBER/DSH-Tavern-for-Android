...
export function entryKeys(v: unknown): string[] {
  if (Array.isArray(v)) return v.map(k => String(k ?? '').trim()).filter(k => k !== '')
  if (typeof v === 'string') return v.split(/[,，]/).map(k => k.trim()).filter(k => k !== '')
  return []
}

/** keys 数组 → 编辑框可读串（逗号分隔；entryKeys 的逆操作展示面） */
export function keysToText(v: unknown): string {
  return entryKeys(v).join(', ')
}

/** 条目是否启用（ST 语义：disabled !== true 即启用） */
export function entryEnabled(entry: LoreEntryLike): boolean {
 ...
 */
export function filterLoreEntries(entries: readonly LoreEntryLike[], query: string): LoreEntryLike[] {
