 ...
 */
export function applyOutputProtocol(text: string, proto: OutputProtocol, streaming = false): ProtocolResult {
  const segs = applyOutputProtocolSegments(text, proto, streaming)
  return {
    display: segs.filter(s => s.kind === 'text').map(s => (s as { content: string }).content).join('').trim(),
    actions: segs.filter(s => s.kind === 'action').map(s => (s as { text: string }).text),
    status: segs.filter(s => s.kind === 'status').map(s => (s as { content: string }).content),
  }
}
...
export type ProtocolSegment =
  ...
  | { kind: 'action'; text: string; color?: string }
  | { kind: 'status'; content: string }
  | { kind: 'collapsible'; title: string; content: string }
  | { kind: 'state-update'; analysis: string | null; patches: string | null; raw: string }
  | { kind: 'reasoning'; content: string }
  | { kind: 'foreshadowing'; content: string }
...
export function parseStateUpdateBlock(inner: string): { analysis: string | null; patches: string | null } {
...
export function parseJsonPatches(jsonText: string): Array<{ op: string; path: string; value?: string }> | null {
  try {
    const arr = JSON.parse(jsonText.trim()) as unknown
 ...
 */
export function splitActionOptions(text: string): ActionOption[] {
  ...
  return groups.map(g => {
    ...
    const label = g
      .replace(/<font\s+color\s*=\s*"(#[0-9a-fA-F]{3,8})"[^>]*>([\s\S]*?)<\/font>/gi,
        (_m, c: string, inner: string) => { color = c; return inner })
      .replace(/<\/?font[^>]*>/gi, '')
 ...
 */
export const HTML_DISPLAY_TAGS: ReadonlySet<string> = new Set([
...
const SVG_ATTRS: ReadonlyMap<string, string> = new Map([
...
export function containsPairedHtml(text: string): boolean {
 ...
 */
export function sanitizeDisplayHtml(html: string): string | null {
  ...
  while ((m = tagRe.exec(html)) !== null) {
    ...
    if (!SANITIZE_TAGS.has(tag)) return null
    ...
    if (sty) {
      ...
      for (const decl of (sty[1] ?? sty[2] ?? sty[3] ?? '').split(';')) {
        ...
        if (!SANITIZE_STYLE_PROPS.has(prop)) return null
        if (/[<>]|url\s*\(|expression\s*\(/i.test(val)) continue
        safe.push(`${prop}: ${val}`)
      }
      if (safe.length > 0) keep += ` style="${safe.join('; ')}"`
    }
    out += `<${tag}${keep}>`
    last = tagRe.lastIndex
  }
...
export function applyOutputProtocolSegments(text: string, proto: OutputProtocol, streaming = false): ProtocolSegment[] {
  ...
  work = work.replace(/<StatusPlaceHolderImpl\s*(?:\/>|>[\s\S]*?<\/StatusPlaceHolderImpl>)/g,
    () => stash({ kind: 'statusbar-placeholder' }))
  ...
  const isUnknownTag = (tag: string): boolean => {
  ...
  const segAsText = (seg: ProtocolSegment | undefined): string => {
  ...
  work = work.replace(/<([A-Za-z][\w~-]*)(?:\s[^<>]*)?\/>/g, (m, tag: string) => (isUnknownTag(tag) ? '' : m))
  ...
  work = work.replace(/<\/([A-Za-z][\w~-]*)>/g, (m, tag: string) =>
    isUnknownTag(tag) || knownTags.has(tag) || knownTags.has(tag.toLowerCase()) ? '' : m)
  ...
  // 相邻文本段合并
  const merged: ProtocolSegment[] = []
  for (const p of parts) {
    const prev = merged[merged.length - 1]
    if (p.kind === 'text' && prev?.kind === 'text') prev.content += p.content
    else merged.push({ ...p })
  }
  // 去掉首尾空白文本段
...
export type TextSegment = { type: 'text'; content: string } | { type: 'statusbar'; content: string }

/**
 * statusbar / status 代码块切分（渲染为组件，不显示原始代码）。
 * 其余文本段交给 MarkdownText；普通代码块留在文本段内正常渲染。
 */
export function splitStatusbarBlocks(text: string): TextSegment[] {
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    if (lang !== 'statusbar' && lang !== 'status') continue
 ...
 */
export function parseStatusBarRows(content: string): StatusRow[] {
  ...
  const rows = t.split('\n').map(l => l.trim()).filter(Boolean).slice(0, 12).map(l => {
    const i = l.search(/[:：]/)
    return i > 0 ? ([l.slice(0, i), l.slice(i + 1).trim()] as StatusRow) : null
  }).filter((r): r is StatusRow => r !== null)
  return rows
}

/** cwd（会话工作区根，如 …/.dsh/rp/<slug>，Windows 反斜杠兼容）→ 工作区 slug */
export function slugFromCwd(cwd: string | undefined): string | null {
  if (!cwd) return null
...
export function withDefaults(p: Partial<OutputProtocol> | undefined): OutputProtocol {
  return {
 ...
 */
export function parseVariableJson(raw: string): { keys: number; pretty: string } | null {
  try {
    const o = JSON.parse(raw.trim()) as unknown
    if (o === null || typeof o !== 'object' || Array.isArray(o)) return null
    return { keys: Object.keys(o as Record<string, unknown>).length, pretty: JSON.stringify(o, null, 2) }
  } catch {
    return null
  }
}
