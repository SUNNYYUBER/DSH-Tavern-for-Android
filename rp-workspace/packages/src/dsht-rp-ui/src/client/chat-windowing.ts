 ...
 */
export function nearBand(rect: RectLike, viewportHeight: number, overscanPx: number): boolean {
  return rect.bottom > -overscanPx && rect.top < viewportHeight + overscanPx
}

/** 进入带缓冲（px）：未渲染条目滚到距视口该距离内即恢复渲染 */
export function enterOverscanPx(viewportHeight: number): number {
  return Math.max(viewportHeight * OVERSCAN_VIEWPORT_RATIO, MIN_OVERSCAN_PX)
}

/** 退出带缓冲（px）：已渲染条目滚出该距离才占位化（> 进入带，滞回防振荡） */
export function exitOverscanPx(viewportHeight: number): number {
 ...
 * delta 恒为 0（绝不抢滚动）。
 */
export function anchorDelta(anchorTopNow: number, scrollTopNow: number, anchorDocTop: number): number {
 ...
 */
export function nextWindowedState(currentWindowed: boolean, forced: boolean, near: boolean): boolean {
  const next = forced ? false : !near
  return next === currentWindowed ? currentWindowed : next
}

/** 占位高度估算：最近实测优先（>0 的有限数），否则默认估算值 */
export function estimatePlaceholderHeight(cached: number | null | undefined, fallback: number = DEFAULT_PLACEHOLDER_HEIGHT): number {
...
export function findScrollAncestor(el: Element): HTMLElement | null {
  ...
  while (node !== null) {
    ...
    if (oy === 'auto' || oy === 'scroll' || oy === 'overlay') return node as HTMLElement
...
// ---------------------------------------------------------------------------
// 共享协调器（模块单例）：registry / 订阅面 / scroll+RO 驱动 / 锚点补偿
// ---------------------------------------------------------------------------
...
interface WindowEntry {
  ...
  nodeKey: string
  el: HTMLElement
  /** 恒渲染真实内容（流式中的消息等）；窗口化永不占位 */
  forced: boolean
  /** 当前是否窗口化（视口外占位） */
  windowed: boolean
  /** 最近实测外壳高度（px；ResizeObserver 维护，供占位估算） */
  height: number | null
  /** 最近一次 windowed 翻转时刻（performance.now()）——RO 补偿跳过翻转后
   * 250ms 内的高度变化（那由翻转批次锚点补偿独家负责，防双重补偿过冲） */
  flipAt: number
}

/** sessionKey → (nodeKey → entry)。占位 div 同样注册（滚回判定需要它的 rect） */
const registry = new Map<string, Map<string, WindowEntry>>()

/** 外壳元素 → entry（ResizeObserver 回调反查） */
const elToEntry = new Map<HTMLElement, WindowEntry>()
...
export function subscribeWindowing(listener: () => void): () => void {
  listeners.add(listener)
  return () => { listeners.delete(listener) }
}

/** 该条消息当前是否窗口化（占位中）。未注册 → false（全渲染兜底） */
export function isMessageWindowed(sessionKey: string, nodeKey: string): boolean {
  return registry.get(sessionKey)?.get(nodeKey)?.windowed === true
}

/** 该条消息的占位高度：最近实测（含跨挂载 memo）优先，否则默认估算值 */
export function messagePlaceholderHeight(sessionKey: string, nodeKey: string): number {
...
export function registerMessageWindowing(reg: {
  ...
  return () => {
    if (entry.height !== null && entry.height > 0) {
      if (heightMemo.size >= HEIGHT_MEMO_LIMIT) heightMemo.clear()
      heightMemo.set(memoKey, entry.height)
    }
    elToEntry.delete(el)
    ro?.unobserve(el)
    entries.delete(nodeKey)
    if (entries.size === 0) registry.delete(sessionKey)
 ...
 */
export function reportWindowCommit(): void {
  if (pendingCompensation === null || compensationQueued) return
...
function onScrollCaptured(): void {
  markScrollActivity()
  scheduleRecompute()
}
...
function markScrollActivity(): void {
  ...
  if (suppressScrollCount > 0) {
    if (now - lastSuppressAt < 1000) { suppressScrollCount--; return } // 补偿写入的 scroll，非用户活动
  ...
  if (registry.size > 0) idleTimer = setTimeout(idleExpandTick, IDLE_SETTLE_MS)
}

/** 程序化 scrollTop 写入前调用（补偿路径），抵消紧随的 scroll 活动标记 */
function suppressNextScrollActivity(): void {
  suppressScrollCount++
  lastSuppressAt = typeof performance !== 'undefined' ? performance.now() : Date.now()
}

function idleExpandTick(): void {
  idleTimer = null
  const now = typeof performance !== 'undefined' ? performance.now() : Date.now()
  if (now - lastActivityAt < IDLE_SETTLE_MS) return // 期间有滚动 → 不扩张
  ...
  if (idleBoostPx < IDLE_BOOST_MAX_PX) idleTimer = setTimeout(idleExpandTick, IDLE_STEP_MS)
}

/** 【临时调试钩子】验证后移除：暴露空闲扩张内部状态 */
if (typeof window !== 'undefined') {
  (window as unknown as Record<string, unknown>).__dshtWinDebug = {
    ...
    get registered() { let n = 0; for (const m of registry.values()) n += m.size; return n },
 ...
 * false）的渐进增高/收缩。 */
const aboveDeltaByContainer = new Map<HTMLElement, number>()
let aboveFlushQueued = false

function flushAboveDeltas(): void {
...
function ensureResizeObserver(): ResizeObserver | null {
  ...
  resizeObserver = new ResizeObserver(items => {
    for (const item of items) {
      const entry = elToEntry.get(item.target as HTMLElement)
      ...
      entry.height = size
      if (prev !== null && Math.abs(size - prev) >= 1 && (item.target as HTMLElement).isConnected) {
        // 【双重补偿根除】翻转（占位↔实测）的高度变化由翻转批次锚点补偿独家负责：
        // 跳过 windowed 条目 + 翻转后 250ms 内的 RO 事件（RO 回调异步到达，
        // 两条路径同帧叠加 ±5740px 曾造成视口在大楼层间震荡——真机取证）。
        if (entry.windowed) continue
        const now = typeof performance !== 'undefined' ? performance.now() : Date.now()
        if (now - entry.flipAt < 250) continue
        const rect = (item.target as HTMLElement).getBoundingClientRect()
        ...
        if (rect.top < 0) {
          ...
          const container = findScrollAncestor(item.target as HTMLElement)
          if (container !== null) {
  ...
  for (const entries of registry.values()) {
    ...
    if (entries.size <= WINDOWING_THRESHOLD) {
      for (const entry of entries.values()) {
        if (entry.windowed) { entry.windowed = false; changed = true }
      }
      continue
    }

    // 1) 同一帧内一次性批读所有 rect（滚动不脏布局，读的是干净布局值）
    const rects = new Map<WindowEntry, RectLike>()
    for (const entry of entries.values()) {
    ...
    let anchor: { entry: WindowEntry; top: number } | null = null
    ...
    if (flipped && anchor !== null) {
      ...
      if (anchor.entry.windowed) {
        let alt: { entry: WindowEntry; top: number } | null = null
        for (const [entry, rect] of rects) {
          if (entry.forced || entry.windowed) continue
          if (rect.bottom > 0 && rect.top < vh && (alt === null || rect.top < alt.top)) alt = { entry, top: rect.top }
        }
        if (alt !== null) anchor = alt
      }
      {
