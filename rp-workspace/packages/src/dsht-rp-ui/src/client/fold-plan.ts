 ...
 */
export function planFolds(rows: readonly FoldRow[]): FoldGroup[] {
  ...
  for (let i = 0; i <= rows.length; i++) {
    ...
    if (isBoundary) {
      if (turnStart >= 0) closeTurn(turnStart, i)
      turnStart = row?.kind === 'user' ? i : -1
    }
  }
  function closeTurn(start: number, end: number): void {
    const slice = rows.slice(start, end) // [user, ..., 收尾?]
    if (slice.length === 0 || slice[slice.length - 1]?.kind !== 'turn-tail') return // 未完成轮
    const tail = slice[slice.length - 1]
    // 最终正文 = 轮内最后一个 assistant-step（不含 turn-tail 自身）
    let lastAssistant = -1
    for (let j = slice.length - 2; j >= 1; j--) {
      if (slice[j]?.kind === 'assistant-step') { lastAssistant = j; break }
    }
    if (lastAssistant === -1) return // 无正文（失败轮等）不折，保留全部可见
    const folded = slice.slice(1, lastAssistant).filter(r => FOLD_KINDS.has(r.kind))
    if (folded.length < FOLD_MIN_ROWS) return
    groups.push({
      id: tail.key,
