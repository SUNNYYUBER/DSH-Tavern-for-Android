...
export interface ExportResult {
  ...
  workspaces: Array<{ slug: string; name: string; dir: string }>
}
...
function parseStDate(s: unknown): number | null {
 ...
 * @param book 已解析的世界书
 */
export function exportLoreBookSkill(book: LoreBook): DshFile[] {
 ...
 * @param card 已解析的角色卡
 */
export function cardPromptPersona(card: CharacterCard): string {
...
// ---------------------------------------------------------------------------
// 聊天 → session.jsonl（严格对照官方契约 oneTurnLog）
// ---------------------------------------------------------------------------
...
export function chatSessionId(characterName: string, chatFile: string): string {
 ...
 */
export const RP_WORKSPACE_DIR = 'rp'
 ...
 * - 结尾无未闭合的 turn/step。
 */
export function assertSessionLogEvents(
  events: ReadonlyArray<{ type: string; seq: number; data?: Record<string, unknown> }>,
): void {
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    switch (ev.type) {
      ...
      case 'turn/end': {
        ...
        if (reason === null || typeof reason !== 'object' || (reason as { kind?: unknown }).kind !== 'completed') {
 ...
 */
export function convertChatFile(
  jsonlText: string,
  opts: { sessionId: string; createdAt: number; cwd?: string; systemHandling?: 'skip' | 'auto' },
): ChatConversion {
  const lines: string[] = []
  let seq = 0
  ...
  if (opts.systemHandling !== 'skip') {
    for (const raw of jsonlText.split('\n')) {
      ...
      if (!trimmed) continue
      try {
        const row = JSON.parse(trimmed) as StChatRow
  ...
  const emit = (
    ...
    const ev: Record<string, unknown> = { type, seq, time, data }
  ...
  const emitAssistantStep = (
    text: string,
    op: 'append' | { op: 'replace'; start: number; end: number },
    sourceEventSeqs?: number[],
  ): number => {
    step++
  ...
  for (const raw of jsonlText.split('\n')) {
    ...
    let row: StChatRow
    try {
      row = JSON.parse(trimmed) as StChatRow
    } catch {
      skipped++
      continue
    }
    const mes = typeof row.mes === 'string' ? row.mes : ''
    if (!mes.trim()) { skipped++; continue }
    if (trustSystemFlag && row.is_system === true) { skipped++; continue }
    ...
    if (row.is_user === true) {
      ...
      const swipes = Array.isArray(row.swipes)
        ? row.swipes.filter((s): s is string => typeof s === 'string' && s.trim().length > 0)
        : []
      ...
      for (let i = 0; i < variants.length; i++) {
        ...
        if (activeSeq === undefined) {
          ...
          activeSeq = emitAssistantStep(text,
            { op: 'replace', start: activeSeq, end: activeSeq }, [activeSeq])
        }
      }
  ...
  try {
    assertSessionLogEvents(
      lines.slice(1).map(l => JSON.parse(l) as { type: string; seq: number; data?: Record<string, unknown> }),
    )
  } catch (e) {
    throw new Error(`convertChatFile(${opts.sessionId}) 构造产出未通过自检：${(e as Error).message}`)
  }
...
export function sessionFilePath(sessionId: string, cwd: string | undefined): string {
  const dir = cwd === undefined ? '_no-cwd' : projectKey(cwd)
  return `sessions/${dir}/${encodeSegment(sessionId)}/session.jsonl`
}

// ---------------------------------------------------------------------------
// 总调度：DataImportSession → DSH 文件集（分批产出，配 bridge 流式写盘）
// ---------------------------------------------------------------------------

/** 单批回调：文件数达到 batchSize 或阶段切换时交付（调用方写盘后继续） */
export type ExportSink = (files: DshFile[], progress: { percent: number; desc: string }) => Promise<void>
 ...
 */
export function buildRpJsonContent(
  card: import('./character-card.ts').CharacterCard,
 ...
 * 重建变体组做开场白左右切换。
 */
export function buildFirstMesSession(
  card: import('./character-card.ts').CharacterCard,
  opts: { cwd?: string; dshHome?: string } = {},
): DshFile | null {
  ...
  const cwd = opts.cwd ?? (opts.dshHome !== undefined
    ? `${opts.dshHome}/${RP_WORKSPACE_DIR}/${dshSlug('rp', card.name)}`
    : `${RP_WORKSPACE_DIR}/${dshSlug('rp', card.name)}`)
  const createdAt = Date.now()
  const lines: string[] = [
    JSON.stringify({ type: 'session', version: 0, id: sessionId, createdAt, cwd, delegationDepth: 0 }),
  ]
  let seq = 0
  const event = (type: string, data: Record<string, unknown>, surfaceOp?: unknown, sourceEventSeqs?: number[]): void => {
    lines.push(JSON.stringify({
  ...
  const emitGreeting = (text: string, step: number, op: 'append' | { op: 'replace'; start: number; end: number }, sources?: number[]): number => {
...
export function exportSingleCardFiles(
  card: import('./character-card.ts').CharacterCard,
  dshHome?: string,
): DshFile[] {
  const files: DshFile[] = []
  const books: Array<{ name: string; lorePath: string }> = []
 ...
 */
export async function exportToDshFiles(
  imp: {
    zip: { files: Record<string, { async(t: 'string'): Promise<string> }> }
    ...
    report?: {
      settings?: {
        ...
        personas?: Array<{ name?: string; description?: string; isDefault?: boolean; avatar?: string | null }>
      }
      relations?: {
  ...
  sink: ExportSink,
  options: ExportToDshOptions = { worlds: true, cards: true, chats: true },
  dshHome?: string,
): Promise<ExportResult> {
  const result: ExportResult = { skills: 0, presets: 0, sessions: 0, files: [], warnings: [], workspaces: [] }
  const BATCH = 40
  let batch: DshFile[] = []
  let percentBase = 0
  let percentSpan = 0

  const flush = async (p: { percent: number; desc: string }) => {
    ...
    if (presetFiles.length > 0) {
      ...
      for (const pf of presetFiles) {
        try {
          ...
          result.warnings.push(`ST 预设导入失败：${pf}（${(e as Error).message}）`)
        }
        done++
        if (batch.length >= BATCH) {
  ...
  if (wantChats || wantCardWorkspaces) {
    ...
    const cardByKey = new Map<string, import('./character-card.ts').CharacterCard>()
    ...
    const lookupCard = (ownerDir: string): import('./character-card.ts').CharacterCard | null =>
      cardByKey.get(ownerDir) ?? cardByKey.get(normNameKey(ownerDir)) ?? null

    const ORPHAN_DIR = '_orphan'

    // R9：全局书单（settings.globalSelect）落盘 rp/global-books.json，并并入每个工作区 rp.json 的 books
    const globalBooks: Array<{ name: string; lorePath: string }> = []
     ...
     */
    const emitCardRpJson = (
      card: import('./character-card.ts').CharacterCard,
      extra: { books: Array<{ name: string; lorePath: string }>; user: string },
    ): void => {
      const books: Array<{ name: string; lorePath: string }> = []
      const seen = new Set<string>()
      const pushBook = (b: { name: string; lorePath: string }) => {
        if (!seen.has(b.lorePath)) { seen.add(b.lorePath); books.push(b) }
      }
      ...
      for (const b of globalBooks) pushBook(b) // R9 全局书单
      for (const b of extra.books) pushBook(b) // R7 聊天绑定书
    ...
    // 每个工作区：README.md（角色说明+聊天清单）+ 该卡全部 session + R7 元数据
    interface WsInfo {
      dir: string
      card: import('./character-card.ts').CharacterCard | null
      chats: Array<{ file: string; sessionPath: string }>
      variantGroups: number
      /** R7：聊天 chat_metadata 绑定的书（lorePath 去重） */
      extraBooks: Map<string, { name: string; lorePath: string }>
      /** R7：last_user_persona.name（首个非空者） */
      userName: string
    }
    const workspaces = new Map<string, WsInfo>()
    const wsFor = (dir: string, card: import('./character-card.ts').CharacterCard | null): WsInfo => {
    ...
    if (wantChats) {
      const orphanDirs = new Map<string, number>()
      for (let i = 0; i < imp.chatFiles.length; i++) {
        ...
        try {
          ...
          const chatCwd = dshHome !== undefined ? `${dshHome}/${RP_WORKSPACE_DIR}/${dir}` : undefined
          ...
          if (meta) {
            const cm = (meta.chat_metadata && typeof meta.chat_metadata === 'object'
              ? meta.chat_metadata : {}) as Record<string, unknown>
            ...
            const lup: unknown = cm.last_user_persona ?? meta.last_user_persona
            const lupName: unknown = typeof lup === 'string'
              ? lup
              : lup && typeof lup === 'object' ? (lup as Record<string, unknown>).name : null
      ...
      if (orphanDirs.size > 0) {
        result.warnings.push(
          `孤儿聊天目录（未匹配到角色卡，已挂 ${RP_WORKSPACE_DIR}/${ORPHAN_DIR} 待认领）：` +
