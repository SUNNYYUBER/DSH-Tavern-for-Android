...
export function convertChatFile(
  ...
  const emitVariantMarker = (shadowedSeq: number, note: string): void => {
...
export function buildFirstMesSession(
  ...
  const emitGreeting = (text: string, step: number): number => {
  ...
  const emitGreetingMarker = (shadowedSeq: number, step: number, n: number, total: number): void => {
    event('step/start', { turn: 1, step })
