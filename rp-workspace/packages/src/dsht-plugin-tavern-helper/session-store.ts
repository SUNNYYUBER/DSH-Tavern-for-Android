...
/** 会话持有层（内存权威 + 快照落盘）；global/character/chat 为共享文件层 */
export type SessionHeldScope = 'preset' | 'message' | 'script'
export const SESSION_HELD_SCOPES: readonly SessionHeldScope[] = ['preset', 'message', 'script']

/** state 文件里快照所在的键 */
export const SNAPSHOT_KEY = 'tavern'
...
export interface TavernSessionSnapshot {
  ...
  readonly scopes: {
    readonly preset: Tree
    readonly message: Tree
  }
...
export function emptySnapshot(): TavernSessionSnapshot {
 ...
 * 调用方回落空快照而不炸路由）。
 */
export function decodeSnapshot(raw: unknown): TavernSessionSnapshot | undefined {
  if (!isTree(raw)) return undefined
  if (raw.format !== 0) return undefined
  const revision = raw.revision
  if (typeof revision !== 'number' || !Number.isSafeInteger(revision) || revision < 0) return undefined
  const scopes = raw.scopes
  if (!isTree(scopes) || !isTree(scopes.preset) || !isTree(scopes.message)) return undefined
  const scripts = raw.scripts
  if (!isTree(scripts)) return undefined
  const parsedScripts: Record<string, Tree> = {}
  for (const [id, tree] of Object.entries(scripts)) {
    if (!isTree(tree)) return undefined
    parsedScripts[id] = tree
  }
  let lastMutation: TavernSessionSnapshot['lastMutation']
  if (raw.lastMutation !== undefined) {
    const m = raw.lastMutation
    if (!isTree(m)) return undefined
    if (m.scope !== 'preset' && m.scope !== 'message' && m.scope !== 'script') return undefined
    if (typeof m.ts !== 'number' || !Number.isFinite(m.ts)) return undefined
    if (m.scriptId !== undefined && typeof m.scriptId !== 'string') return undefined
    lastMutation = {
      scope: m.scope,
...
export function readSnapshotFromStateFile(file: Tree | null): TavernSessionSnapshot | undefined {
  if (file === null) return undefined
  return decodeSnapshot(file[SNAPSHOT_KEY])
}

/** 把快照写进 state 文件对象（只替换 tavern 键，其余键原样保留；不改入参） */
export function writeSnapshotIntoStateFile(file: Tree, snapshot: TavernSessionSnapshot): Tree {
  return { ...file, [SNAPSHOT_KEY]: snapshot }
}

/** 应用一次整树替换（不可变：返回新快照；revision+1 并记录 lastMutation） */
export function applySnapshotMutation(
  snapshot: TavernSessionSnapshot,
...
/** state 文件整读整写适配（index.ts 注入；测试可用内存实现） */
export interface SessionStoreIO {
  /** 读 rp/state/<sid>.json 整文件；无文件/坏 JSON → null */
  readStateFile(sessionId: string): Promise<Tree | null>
  /** 整写 rp/state/<sid>.json（调用方保证除 tavern 键外其余键来自最新读盘结果） */
  writeStateFile(sessionId: string, file: Tree): Promise<void>
}
 ...
 */
export class TavernSessionStore {
  readonly #io: SessionStoreIO
  readonly #cache = new Map<string, TavernSessionSnapshot>()

  constructor(io: SessionStoreIO) {
    this.#io = io
  }

  async get(sessionId: string): Promise<TavernSessionSnapshot> {
  ...
  async mutate(sessionId: string, mutation: SnapshotMutation): Promise<TavernSessionSnapshot> {
