...
exports.writeCardTextChunks = writeCardTextChunks;
exports.bytesToBase64 = bytesToBase64;
exports.exportCardBundleFiles = exportCardBundleFiles;
exports.buildStV2Json = buildStV2Json;
exports.loreBookToStWorld = loreBookToStWorld;
exports.makePlaceholderPng = makePlaceholderPng;
// ---------------------------------------------------------------------------
...
function writeCardTextChunks(png, jsonText) {
    if (png.length < 8)
        throw new Error('PNG 字节过短（<8），非合法 PNG');
    const sig = [0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A];
    for (let i = 0; i < 8; i++) {
        if (png[i] !== sig[i])
            throw new Error('PNG 签名不匹配（非合法 PNG）');
    }
    ...
    if (iendPos === -1)
        throw new Error('PNG 无 IEND chunk，无法写入');
    ...
    while (pos + 12 <= iendPos) {
        ...
        if (type === 'tEXt') {
            ...
            let kwEnd = dataStart;
            while (kwEnd < dataStart + length && png[kwEnd] !== 0)
                kwEnd++;
