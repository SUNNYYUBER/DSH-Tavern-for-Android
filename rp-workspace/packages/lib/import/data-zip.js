...
exports.lastDataImport = lastDataImport;
exports.extractPersonas = extractPersonas;
exports.analyzeDataZip = analyzeDataZip;
const jszip_1 = __importDefault(require("jszip"));
const entry_ts_1 = require("../lore/entry.ts");
const character_card_ts_1 = require("./character-card.ts");
...
function detectRoot(files) {
    ...
    for (const c of candidates) {
        const prefix = c === '' ? '' : c + '/';
        const score = ST_MARKERS.filter(m => normed.some(f => f === prefix + m || f.startsWith(prefix + m + '/'))).length;
...
function extractPersonas(sj) {
    ...
    if (defaultName === null && personas.length > 0)
    ...
    if (personas.length === 0) {
        ...
        if (legacy && typeof legacy === 'object') {
            ...
            if (name)
                personas.push({ name, description: desc, isDefault: true, avatar: null });
        }
    }
    return personas;
}
...
async function analyzeDataZip(zipData, onProgress) {
    ...
    for (let i = 0; i < worldFiles.length; i++) {
        ...
        try {
            ...
            const book = (0, entry_ts_1.importLoreBook)(name, JSON.parse(text));
            worlds.set(name, book);
            report.stats.worldBooks++;
            report.stats.loreEntries += book.entries.length;
            for (const w of book.importWarnings.slice(0, 3))
                report.warnings.push(`「${name}」${w}`);
        }
        catch {
    ...
    const avatarByDir = new Map();
    for (const a of avatarCandidates) {
        const dir = a.split('/').slice(0, -1).join('/');
        if (!avatarByDir.has(dir))
            avatarByDir.set(dir, a);
    }
    let charDone = 0;
        ...
        if (/\.(png|json)$/i.test(f)) {
            ...
            try {
                ...
                if (/\.png$/i.test(f)) {
                    ...
                    card = (0, character_card_ts_1.importCharacterPng)(buf, name);
                }
                else {
                    const text = await zip.files[zipKey(f)].async('string');
