...
exports.repairSessionForV3 = repairSessionForV3;
// @adapt contract:session.v0-legacy-repair
const th_floors_ts_1 = require("./th-floors.ts");
...
function repairSessionForV3(content) {
    ...
    while (lines.length > 0 && lines[lines.length - 1].trim() === '')
        lines.pop();
    if (lines.length === 0)
        return { content, changed: false, notes: [], events: 0, salvaged: [], error: '空文件' };
    let header;
    try {
    ...
    if (header?.type !== 'session')
        return { content, changed: false, notes: [], events: 0, salvaged: [], error: '首行不是 session header' };
    const notes = [];
    let changed = false;
    ...
    for (const item of raw) {
        ...
        if (ev.type === 'user/message') {
            ...
            if (r.dropped !== undefined)
                pendingSalvage.push({ ev, payload: r.dropped });
        }
    ...
    if (fixPruneSurfaceSpans(out, notes))
        changed = true;
    ...
    if (normalizeStructure(out, notes))
        changed = true;
    if (!changed)
        return { content, changed: false, notes: [], events: out.length, salvaged: [] };
    // ---- 4) 重编号 ----
    ...
    const mapRef = (q) => {
        if (idRemap.has(q))
            return idRemap.get(q);
        let best = -1;
        for (const [old, neu] of idRemap)
            if (old <= q && old > best)
                best = old;
    ...
    for (let i = 0; i < out.length; i++) {
        ...
        if (ev.sourceEventSeqs)
            ev.sourceEventSeqs = uniqueSorted(ev.sourceEventSeqs.map(mapRef)).filter(q => q < ev.seq);
        ...
        if (ev.type === 'compaction/prune') {
            ...
            if (Array.isArray(d.shadowedSeqs))
                d.shadowedSeqs = uniqueSorted(d.shadowedSeqs.map(mapRef)).filter(q => q < ev.seq);
    ...
    // title 引用对齐（必须在重编号之后）
    if (fixTitleMessageSeqs(out, mapRef, notes))
        changed = true;
    ...
    for (const { ev, payload } of pendingSalvage) {
        if (!alive.has(ev))
            continue;
        const key = (0, th_floors_ts_1.thFloorKeyOf)(ev.data, ev.seq);
        if (key !== null)
            salvaged.push({ key, payload });
    }
    // ---- 6) 序列化（信封只输出白名单键） ----
    ...
    for (const ev of events) {
        ...
        switch (ev.type) {
            ...
            case 'turn/end': {
                ...
                if (openStep !== null)
                    closeStep(openTurn, openStep); // 先补 step/end
                out.push(ev);
                openTurn = null;
                nextTurn += 1;
                break;
...
function fixTitleMessageSeqs(events, mapRef, notes) {
    let changed = false;
    for (const ev of events) {
        if (ev.type !== 'session/title' && ev.type !== 'session/title-llm-request')
            continue;
        const d = ev.data;
    ...
    if (changed)
        notes.push('session/title 的 messageSeqs 重新对齐（必须引用更早的真人 user 消息）');
    return changed;
}
// ---------------------------------------------------------------- 子修复
/**
...
function normalizePluginForm(s, notes) {
    ...
    if (changed)
        notes.push('plugin source 的 form 与 sections/summary 形态对齐（snapshot 必须有 sections）');
    return changed;
}
/**
...
function fixUserMessage(data, notes) {
    ...
    if (src !== null && typeof src === 'object') {
        ...
        if (s.kind === 'plugin' && normalizePluginForm(s, notes))
            changed = true;
        d.source = s;
    }
...
function serialize(ev) {
    ...
    if (ev.surfaceOp !== undefined)
        o.surfaceOp = ev.surfaceOp;
    if (ev.sourceEventSeqs !== undefined)
        o.sourceEventSeqs = ev.sourceEventSeqs;
...
function expandPackedRow(row) {
    ...
    if (typeof seq0 !== 'number' || typeof time0 !== 'number' || !Array.isArray(payload))
        return [];
 ...
 */
function planTurnReassign(events) {
    let nextTurn = 1;
    let openTurn = null;
    const plan = new Map();
    events.forEach((ev, i) => {
        ...
        if (ev.type === 'step/start' || ev.type === 'step/end' || ev.type === 'assistant/message') {
            if (openTurn !== null && typeof ev.data.turn === 'number' && ev.data.turn !== openTurn)
                plan.set(i, openTurn);
        }
    });
    return plan;
