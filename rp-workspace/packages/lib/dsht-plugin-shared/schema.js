...
exports.SCHEMA_MAX_DEPTH = void 0;
exports.validateSchemaSubset = validateSchemaSubset;
/** 递归深度上限（≤5 层；超过放行——防循环 schema 拖死校验） */
exports.SCHEMA_MAX_DEPTH = 5;
/** 最小子集校验（返回问题清单；空数组 = 通过） */
function validateSchemaSubset(value, schema, path = '$', depth = 0) {
    if (schema === null || typeof schema !== 'object' || Array.isArray(schema))
        return [];
    if (depth >= exports.SCHEMA_MAX_DEPTH)
        return [];
    const s = schema;
