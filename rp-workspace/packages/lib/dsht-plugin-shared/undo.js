...
exports.undoLogPath = undoLogPath;
exports.makeUndoEntry = makeUndoEntry;
exports.appendUndoEntries = appendUndoEntries;
exports.readUndoLog = readUndoLog;
exports.clearUndoLog = clearUndoLog;
exports.diffUndoEntries = diffUndoEntries;
exports.replayUndoLog = replayUndoLog;
...
function makeUndoEntry(scope, slug, path, tree, ts = Date.now()) {
    const pointer = (0, macros_ts_1.toPointer)(path);
    const oldValue = (0, macros_ts_1.readVarPath)(tree, pointer);
...
async function appendUndoEntries(dshHome, sessionId, entries) {
    ...
    await (0, promises_1.appendFile)(file, entries.map(e => JSON.stringify(e)).join('\n') + '\n', 'utf8');
}
/** 读取 undo 日志（坏行跳过；无文件 = 空） */
async function readUndoLog(dshHome, sessionId) {
    try {
        const text = await (0, promises_1.readFile)(undoLogPath(dshHome, sessionId), 'utf8');
        const out = [];
        for (const line of text.split('\n')) {
            ...
            try {
                ...
                if (typeof e?.ts === 'number' && typeof e?.path === 'string' && typeof e?.scope === 'string')
                    out.push(e);
            }
            catch { /* 坏行跳过 */ }
        }
...
async function clearUndoLog(dshHome, sessionId) {
    await (0, promises_1.rm)(undoLogPath(dshHome, sessionId), { force: true });
}
...
function diffUndoEntries(scope, slug, before, after, ts = Date.now()) {
    ...
    for (const [path, old] of beforeFlat) {
        if (!afterFlat.has(path))
            entries.push({ ts, scope, slug, path, oldValue: old, had: true });
    }
    return entries;
}
// ---------------------------------------------------------------------------
...
async function loadScopeTree(dshHome, scope, slug, sessionId) {
    const file = scope === 'global'
        ? (0, node_path_1.join)(dshHome, 'rp', 'variables', 'global.json')
        : scope === 'character'
            ? (0, node_path_1.join)(dshHome, 'rp', slug, 'variables.json')
            : (0, node_path_1.join)(dshHome, 'rp', 'state', `${sessionId}.json`);
    try {
...
async function replayUndoLog(dshHome, sessionId, cutoffTs = Number.NEGATIVE_INFINITY) {
    ...
    for (const e of toReplay) {
        ...
        bucket.tree = (0, macros_ts_1.writeVarPath)(bucket.tree, e.path, e.had ? e.oldValue : undefined);
        bucket.dirty = true;
    }
