...
exports.flushEntryPuts = flushEntryPuts;
...
exports.activateWorldInfo = activateWorldInfo;
exports.assembleGenerateRawPrompt = assembleGenerateRawPrompt;
...
async function context(dshHome, body) {
    ...
    if (presetId !== null) {
        try {
            ...
            if (presetId === 'rp-demo-direct')
                preset = (0, demo_ts_1.demoDirectPreset)();
            else if (presetId === 'rp-demo-light-agent')
                preset = (0, demo_ts_1.demoLightAgentPreset)();
            else
                preset = null;
        }
    }
...
let sessionHeadersCache = null;
const SESSION_HEADERS_TTL_MS = 3000;
async function scanSessionHeadersCached(dshHome) {
    const now = Date.now();
    if (sessionHeadersCache !== null && now - sessionHeadersCache.at < SESSION_HEADERS_TTL_MS)
        return sessionHeadersCache.value;
    const value = await (0, session_surgery_ts_1.scanSessionHeaders)(dshHome);
...
async function chatMessages(dshHome, body) {
    ...
    if (!hit) {
        const fresh = await (0, session_surgery_ts_1.scanSessionHeaders)(dshHome);
    ...
    const rl0 = (0, node_readline_1.createInterface)({ input: (0, node_fs_1.createReadStream)(logPath, 'utf8'), crlfDelay: Infinity });
    for await (const line of rl0) {
        ...
        if (Array.isArray(d?.shadowedSeqs))
            for (const q of d.shadowedSeqs)
                if (typeof q === 'number')
                    shadowed.add(q);
    }
    // 第二遍：导出（跳过被遮蔽事件）
    const rl = (0, node_readline_1.createInterface)({ input: (0, node_fs_1.createReadStream)(logPath, 'utf8'), crlfDelay: Infinity });
    for await (const line of rl) {
        ...
        if (typeof ev.seq === 'number' && shadowed.has(ev.seq))
            continue;
        let role;
        let msg;
...
async function regexesReplace(dshHome, body) {
    ...
    if (scope === 'preset') {
        ...
        await snapshotFor(dshHome, sessionId, [relPath]);
        await atomicWrite((0, node_path_1.join)(dir, 'regex.json'), JSON.stringify({ scripts }, null, 1), 'utf8');
...
async function locateBook(dshHome, name) {
    ...
    try {
        for (const dir of (await (0, promises_1.readdir)((0, node_path_1.join)(dshHome, 'skills'))).sort()) {
            ...
            const lorePath = `skills/${dir}/references/lore.json`;
            if (dir === name) {
                locateBookCache.set(name, lorePath);
                return lorePath;
            }
            try {
                const parsed = JSON.parse(await (0, promises_1.readFile)(homePath(dshHome, lorePath), 'utf8'));
        ...
        if (Array.isArray(g.books)) {
            for (const b of g.books.filter(isTree)) {
                if (b.name === name && typeof b.lorePath === 'string' && b.lorePath) {
                    locateBookCache.set(name, b.lorePath);
                    return b.lorePath;
                }
            }
        }
...
const TH_POSITION_TYPE_TO_ST = {
    before_char: entry_ts_1.WI_POSITION.BEFORE,
    after_char: entry_ts_1.WI_POSITION.AFTER,
    before_authors_note: entry_ts_1.WI_POSITION.AN_TOP,
    after_authors_note: entry_ts_1.WI_POSITION.AN_BOTTOM,
    at_depth: entry_ts_1.WI_POSITION.AT_DEPTH,
    before_example_messages: entry_ts_1.WI_POSITION.EM_TOP,
    after_example_messages: entry_ts_1.WI_POSITION.EM_BOTTOM,
};
function stEntryToLore(st, bookName, id) {
    ...
    if (position === -1 && st.position !== null && typeof st.position === 'object' && !Array.isArray(st.position)) {
        const po = st.position;
        const mapped = typeof po.type === 'string' ? TH_POSITION_TYPE_TO_ST[po.type] : undefined;
        position = mapped !== undefined ? mapped : entry_ts_1.WI_POSITION.BEFORE;
    }
    else if (position === -1)
    ...
    return {
        ...
        keys: arr(st.key ?? st.keys ?? (po ? po.keys ?? po.strategy?.keys : undefined)),
        secondaryKeys: arr(st.keysecondary ?? st.secondaryKeys ?? (po ? po.strategy?.secondary_keys : undefined)),
        selectiveLogic: num(st.selectiveLogic ?? (po ? po.strategy?.selective_logic : undefined), 0),
...
async function worldbookGet(dshHome, body) {
    ...
    try {
        ...
        if (hit && hit.mtimeMs === st.mtimeMs && hit.size === st.size) {
            ...
            book = JSON.parse(await (0, promises_1.readFile)(abs, 'utf8'));
            if (loreBookCache.size > 8)
                loreBookCache.clear();
...
async function flushEntryPuts(dshHome, lorePath) {
    ...
    const run = prev.catch(() => { }).then(async () => {
        ...
        try {
            const st = await (0, promises_1.stat)(homePath(dshHome, lorePath));
...
function activateWorldInfo(entries, historyText, userInput) {
    ...
    for (const e of entries) {
        ...
        const bucket = e.position === entry_ts_1.WI_POSITION.BEFORE ? before : after;
...
async function primaryCharacterBook(dshHome, slug) {
    if (!slug)
        return null;
    try {
        const dirs = (await (0, promises_1.readdir)((0, node_path_1.join)(dshHome, 'rp', slug, 'books'), { withFileTypes: true }))
            .filter(d => d.isDirectory()).map(d => d.name).sort();
        for (const dir of dirs) {
            try {
                await (0, promises_1.stat)(homePath(dshHome, `rp/${slug}/books/${dir}/lore.json`));
                return dir;
            }
            catch {
                continue;
...
async function loadCardFields(dshHome, slug) {
    if (!slug)
        return {};
    try {
        const parsed = JSON.parse(await (0, promises_1.readFile)(homePath(dshHome, `rp/${slug}/card.json`), 'utf8'));
        const root = isTree(parsed) ? parsed : {};
        return isTree(root.data) ? { ...root, ...root.data, ...root } : root;
    }
    catch {
        return {};
    }
...
async function assembleGenerateRawPrompt(dshHome, body) {
    ...
    const [persona, card, primaryBook] = await Promise.all([
        (0, macros_ts_1.loadActivePersona)(dshHome),
    ...
    for (const name of bookNames) {
        try {
            ...
            const loreEntries = entries.map(e => ({
                ...
                position: typeof e.position === 'number' ? e.position : entry_ts_1.WI_POSITION.BEFORE,
