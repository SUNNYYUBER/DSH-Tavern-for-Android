...
import { dirname as dirname3, join as join5, relative, resolve } from "node:path";
...
async function loadActivePersona(dshHome) {
  try {
    const parsed = JSON.parse(await readFile3(join4(dshHome, "rp", "persona.json"), "utf8"));
...
async function runMacroExpand(deps, input) {
  ...
  try {
    const disk = JSON.parse(await readFile3(join4(deps.dshHome, "rp", "macros.json"), "utf8"));
...
async function listPresets(dshHome) {
  ...
  let dirs = [];
  try {
    dirs = await readdir3(join5(dshHome, "rp-presets"));
  } catch {
    return out;
  }
  for (const id of dirs.sort()) {
    try {
      const p = JSON.parse(await readFile4(join5(dshHome, "rp-presets", id, "preset.json"), "utf8"));
...
async function resolveSessionPresetId(dshHome, sessionId) {
  try {
    const file = JSON.parse(await readFile4(join5(dshHome, "rp", "state", `${sessionId}.json`), "utf8"));
    const pid = presetIdFromStateFile(file);
    if (pid) return pid;
  } catch {
  }
  try {
    const batches = (await readdir3(join5(dshHome, "rp-import"))).filter((b) => /^[a-z0-9][a-z0-9-]{0,60}$/.test(b)).sort().reverse();
    for (const b of batches) {
      const dir = join5(dshHome, "rp-import", b);
      let stRoot = "data/default-user";
      try {
        const meta = JSON.parse(await readFile4(join5(dir, "meta.json"), "utf8"));
        if (typeof meta.manifest?.stRoot === "string" && meta.manifest.stRoot) stRoot = meta.manifest.stRoot;
      } catch {
      }
      let activeName = null;
      try {
        activeName = activePresetNameFromSettings(JSON.parse(await readFile4(join5(dir, "unpacked", stRoot, "settings.json"), "utf8")));
      } catch {
        continue;
      }
...
function homePath(dshHome, relPath) {
  return join5(dshHome, ...relPath.split("/"));
}
...
async function presetExport(dshHome, body) {
  ...
  const regexScripts = [];
  try {
    const parsed = JSON.parse(await readFile4(homePath(dshHome, `rp-presets/${hit.id}/regex.json`), "utf8"));
...
async function presetDelete(dshHome, body) {
  ...
  await rm3(join5(dshHome, "rp-presets", presetId), { recursive: true, force: true });
...
function rpSlugFromCwd(dshHome, cwd) {
  if (!cwd) return null;
  const rel = relative(join5(dshHome, "rp"), resolve(cwd));
  if (!rel || rel.startsWith("..")) return null;
  const slug = rel.split(/[\\/]/)[0];
  return slug || null;
}
var chatMessagesCache = /* @__PURE__ */ new Map();
var sessionHeadersCache = null;
var SESSION_HEADERS_TTL_MS = 3e3;
...
async function chatMessages(dshHome, body) {
  ...
  const logPath = join5(dshHome, "sessions", hit.project, hit.sdir, "session.jsonl");
  let cached;
  try {
  ...
  for await (const line of rl0) {
    ...
    if (Array.isArray(d?.shadowedSeqs)) {
      for (const q of d.shadowedSeqs) if (typeof q === "number") shadowed.add(q);
    }
  }
  ...
  for await (const line of rl) {
    ...
    messages.push({
      ...
      name: role2 === "user" ? "User" : role2 === "system" ? "System" : charName,
      role: role2,
      message: text,
...
async function regexesReplace(dshHome, body) {
  ...
  if (scope === "preset") {
    ...
    const dir = join5(dshHome, "rp-presets", presetId);
    try {
      await readdir3(dir);
    } catch {
    ...
    await snapshotFor(dshHome, sessionId, [relPath]);
    await atomicWrite(join5(dir, "regex.json"), JSON.stringify({ scripts }, null, 1), "utf8");
...
async function worldbookList(dshHome, _body) {
  ...
  try {
    for (const dir of (await readdir3(join5(dshHome, "skills"))).sort()) {
...
async function locateBook(dshHome, name2) {
  ...
  try {
    const dir = join5(dshHome, "rp", "chat-worldbooks");
...
var loreBookCache = /* @__PURE__ */ new Map();
...
async function primaryCharacterBook(dshHome, slug) {
  if (!slug) return null;
  try {
    const dirs = (await readdir3(join5(dshHome, "rp", slug, "books"), { withFileTypes: true })).filter((d) => d.isDirectory()).map((d) => d.name).sort();
  ...
  registerPrefix(ctx, "/dsht-tavern-helper", "dsht-th", async (sub, req, res) => {
    ...
    if (sub === "/chat/append" || sub === "/chat/update") {
      ...
      try {
        ...
        return sendJson(res, 502, { error: `\u4F1A\u8BDD\u5199\u6865\u8F6C\u53D1\u5931\u8D25\uFF1A${e.message}` });
      }
    }
