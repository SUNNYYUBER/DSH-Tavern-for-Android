...
import { entryEnabled, entryKeys, filterLoreEntries, keysToText } from './lore-view.ts';
export function RpLorePanel(props) {
    ...
    useEffect(() => {
        let alive = true;
        void loadEjsDisplaySettings().then(s => { if (alive && s !== null)
            setCodeEditor(s.codeEditor); });
        return () => { alive = false; };
    }, []);
    // 书列表（进入面板拉一次）
    useEffect(() => {
        thApi('worldbook/list', {})
            .then(r => setBooks(r.books ?? []))
            .catch(e => setStatus(`书列表加载失败：${e.message}`));
    }, []);
    /** 选书/保存后重拉条目（get 永远返回全量，uid = 数组下标稳定） */
    const loadEntries = (name) => {
        thApi('worldbook/get', { name })
            .then(r => setEntries((r.entries ?? [])))
            .catch(e => setStatus(`条目加载失败：${e.message}`));
    };
    const openBook = (name) => {
        setSelected(name);
        setEntries([]);
    ...
    return (<div className="dsht-rp-lore">
      {(onBack !== undefined || inBook) && (<div className="lore-toolbar">
          <button type="button" className="dsht-rp-btn lore-btn" onClick={() => {
                if (inBook) {
                    setSelected(null);
                    setEntries([]);
                    setStatus('');
          ...
          {inBook && (<input className="dsht-rp-field lore-search" placeholder="搜索 标题/内容/关键词…" value={query} onChange={e => { setQuery(e.target.value); }}/>)}
        </div>)}
      ...
      {!inBook && (<>
          ...
          {books.length === 0 && status === '' && (<div className="dsht-rp-empty">书库为空——先在「导入」页导入世界书（或整包迁移 ST 数据）。</div>)}
          {books.map(b => (<button key={b.lorePath ?? b.name} type="button" className="wb-row" onClick={() => { openBook(b.name); }}>
      ...
      {inBook && (<>
          ...
          {entries.length > 0 && visible.length === 0 && (<div className="dsht-rp-empty">没有匹配「{query}」的条目。</div>)}
          {visible.map(entry => {
                const open = expandedUid === entry.uid;
                const enabled = entryEnabled(entry);
                const keyText = entryKeys(entry.key).join('、');
                return (<div key={entry.uid} className="lore-entry" data-disabled={!enabled || undefined}>
                <div className="lore-entry-head">
                  <label className="rx-toggle" title={enabled ? '点击停用' : '点击启用'}>
                    <input type="checkbox" checked={enabled} disabled={busy} onChange={() => { toggleEnabled(entry); }}/>
                    <span />
                  </label>
                  <button type="button" className="lore-name" title="点击展开编辑" onClick={() => { open ? closeEdit() : beginEdit(entry); }}>
                ...
                {open && draft !== null && (<div className="lore-editor">
                    <label className="pe-field"><span>标题（comment）</span>
                      <input className="dsht-rp-field" value={draft.comment} onChange={e => { setDraft({ ...draft, comment: e.target.value }); }}/>
                    </label>
                    <label className="pe-field"><span>主关键词（逗号分隔）</span>
                      <input className="dsht-rp-field" value={draft.keys} onChange={e => { setDraft({ ...draft, keys: e.target.value }); }}/>
                    </label>
                    <label className="pe-field"><span>次关键词（可选，逗号分隔）</span>
                      <input className="dsht-rp-field" value={draft.keysSecondary} onChange={e => { setDraft({ ...draft, keysSecondary: e.target.value }); }}/>
                    </label>
                    <label className="pe-field"><span>内容（content）</span>
                      {codeEditor
                            // B17：轻量行号编辑器（EJS 标签行提示 + Tab 缩进；10 行起，随内容自适应加高）
                            ? <CodeTextarea value={draft.content} onChange={next => { setDraft({ ...draft, content: next }); }} rows={Math.max(10, Math.min(24, draft.content.split('\n').length + 1))} ariaLabel="世界书条目内容"/>
                            : <textarea className="dsht-rp-field lore-content" rows={5} value={draft.content} onChange={e => { setDraft({ ...draft, content: e.target.value }); }}/>}
                    </label>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button type="button" className="dsht-rp-btn" style={{ height: 30, fontSize: 13 }} disabled={busy} onClick={() => {
                      ...
                      <button type="button" className="dsht-rp-btn" style={{ height: 30, fontSize: 13, background: 'var(--dsw-alias-interactive-bg-hover)', color: 'var(--dsw-alias-label-primary)' }} onClick={closeEdit}>取消</button>
                    </div>
                  </div>)}
              </div>);
            })}
