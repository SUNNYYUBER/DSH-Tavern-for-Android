...
export function CodeTextarea(props) {
    ...
    return (<div className="dsht-code-ta" data-testid="dsht-code-textarea">
      ...
      <div className="dct-body" style={{ height: `${rows * 20 + 12}px` }}>
        <div className="dct-gutter" ref={gutterRef} aria-hidden="true">
          {Array.from({ length: lineCount }, (_, i) => (<div key={i} className={'dct-ln' + (ejsLines.has(i + 1) ? ' dct-ln-ejs' : '')}>{i + 1}</div>))}
        </div>
        <textarea ref={taRef} className="dct-input" value={value} rows={rows} placeholder={placeholder} aria-label={ariaLabel} spellCheck={false} wrap="off" onChange={e => { onChange(e.target.value); }} onScroll={syncScroll} onKeyDown={onKeyDown}/>
      </div>
    </div>);
}
