...
import { SEARCH_RESULT_LIMIT, searchMessages, splitHighlight, } from './search-core.ts';
/** 输入去抖（ms） */
const DEBOUNCE_MS = 300;
export function RpSearchPanel(props) {
    ...
    const expanded = useMemo(() => (expandedId === null ? null : (messages ?? []).find(m => m.message_id === expandedId) ?? null), [messages, expandedId]);
    const onKeyDown = (ev) => {
        if (ev.key === 'Enter') {
            ev.preventDefault();
            setDebounced(query);
    ...
    return (<div className="dsht-rp-searchview-mask" role="dialog" aria-label="消息搜索" onClick={onClose}>
      <div className="dsht-rp-searchview" onClick={e => { e.stopPropagation(); }}>
        ...
        <div className="se-searchbar">
          <input ref={inputRef} className="dsht-rp-field se-input" type="text" placeholder="搜索消息文本（大小写不敏感）…" value={query} onChange={e => { setQuery(e.target.value); setExpandedId(null); }} onKeyDown={onKeyDown}/>
          {debounced !== '' && !error && messages !== null && (<span className="se-count">
        ...
        <div className="sv-body se-body">
          ...
          {!error && messages !== null && debounced === '' && (<div className="sv-empty">输入关键词搜索本会话的全部消息。</div>)}
          {!error && messages !== null && debounced !== '' && hits.length === 0 && (<div className="sv-empty">没有匹配「{debounced}」的消息。</div>)}
          {hits.map(h => (<button key={h.messageId} type="button" className="se-hit" aria-expanded={expandedId === h.messageId} onClick={() => { setExpandedId(cur => (cur === h.messageId ? null : h.messageId)); }}>
              ...
              {expandedId === h.messageId && expanded !== null && (<span className="se-full">
                  {splitHighlight(expanded.message, debounced).map((seg, i) => seg.hit ? <mark key={i} className="se-mark">{seg.t}</mark> : <span key={i}>{seg.t}</span>)}
                </span>)}
            </button>))}
        </div>
      </div>
