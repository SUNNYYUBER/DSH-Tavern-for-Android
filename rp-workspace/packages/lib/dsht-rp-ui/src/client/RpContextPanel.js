...
export function RpContextPanel({ sessionId, onClose }) {
    ...
    const commit = useCallback(async (patch) => {
        ...
        setErr('');
        try {
            const r = await memApi('settings', patch);
    ...
    const expand = useCallback(async () => {
        ...
        setNote('');
        try {
            const r = await memApi('expand', { sessionId });
      ...
      <div className="dsht-rp-ctx-panel" data-testid="dsht-rp-ctx-panel">
        ...
        <div className="cp-row">
          ...
          <div className="cp-stepper">
            ...
            <input value={floors} inputMode="numeric" disabled={busy} onChange={e => setFloors(e.target.value)} onBlur={e => commitFloors(e.target.value)} onKeyDown={e => { if (e.key === 'Enter')
        commitFloors(e.target.value); }}/>
            <button type="button" disabled={busy} onClick={() => commitFloors(String(clamp((Number(floors) || 0) + FLOOR_STEP, FLOOR_MIN, FLOOR_MAX)))}>+</button>
          </div>
        </div>
        <div className="cp-row">
          ...
          <div className="cp-stepper">
            ...
            <input value={budget} inputMode="numeric" disabled={busy} onChange={e => setBudget(e.target.value)} onBlur={e => commitBudget(e.target.value)} onKeyDown={e => { if (e.key === 'Enter')
