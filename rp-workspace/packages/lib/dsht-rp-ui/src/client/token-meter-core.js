...
export function sumMessageChars(messages) {
    let sum = 0;
    for (const m of messages ?? []) {
        if (typeof m?.message === 'string')
            sum += m.message.length;
    }
    return sum;
}
/** 占用百分比（0-100，预算非法/为 0 时返回 0——进度条空转而非 NaN） */
export function meterPercent(tokens, budget) {
...
export function meterLevel(tokens, budget) {
    const pct = meterPercent(tokens, budget);
    if (pct > DANGER_RATIO * 100)
        return 'danger';
