...
export function RpTablesView(props) {
    ...
    const fetchSheets = useCallback(async () => {
        if (!sessionId)
            return;
        try {
            const r = await memGet('tables', `sessionId=${encodeURIComponent(sessionId)}`);
    ...
    return (<div className="dsht-rp-stateview-mask" role="dialog" aria-label="剧情表格" onClick={onClose}>
      <div className="dsht-rp-stateview" onClick={e => { e.stopPropagation(); }}>
        ...
        <div className="sv-body">
          ...
          {!error && sheets !== null && active.length === 0 && (<div className="sv-empty">（本会话暂无启用表格——表格由剧情/指令逐步生成）</div>)}
          {active.map(s => {
            const isCollapsed = collapsed.has(s.uid);
            return (<div key={s.uid} style={{ marginBottom: 12 }}>
                {/* 顶部折叠行（点击切换；▾ 展开 / ▸ 折叠） */}
                <button type="button" className="sv-row sv-node" aria-expanded={!isCollapsed} style={{ display: 'flex', width: '100%' }} onClick={() => { toggle(s.uid); }}>
                ...
                {!isCollapsed && s.headers.length > 0 && (<table style={{ borderCollapse: 'collapse', width: '100%', marginTop: 4 }}>
                    <thead>
                      <tr>
