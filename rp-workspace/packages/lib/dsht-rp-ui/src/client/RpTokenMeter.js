...
import { DEFAULT_MAX_CONTEXT, estimateTokens, formatTokens, meterLevel, meterPercent, sumMessageChars, } from './token-meter-core.ts';
...
function fetchBudget(sessionId) {
    ...
    if (p === undefined) {
        p = (async () => {
            try {
                const rg = await thApi('regexes/get', { sessionId });
                const presetId = rg.presetId ?? '';
                if (!presetId)
                    return DEFAULT_MAX_CONTEXT;
                const pl = await rpApi('preset/list');
...
export function RpTokenMeter(props) {
    ...
    useEffect(() => {
        ...
        const pull = async () => {
            try {
                const r = await thApi('chat/messages', { sessionId });
                if (alive)
                    setChars(sumMessageChars(r.messages));
            }
            catch { /* 保持上次值 */ }
        };
        void pull();
    ...
    return (<div className="dsht-rp-tokenmeter" data-level={level}>
      <button type="button" className="tm-hit" title={real !== null
            ? '≈ 上下文占用（真实 provider usage：最近一次请求的 prompt 实测值；点击调节 AI 可见楼层数 / 展开折叠区间）'
            : '≈ 上下文占用（字符量估算：总字数 ÷ 2.5，首轮回复落地后改显 provider 真实 usage；点击调节 AI 可见楼层数 / 展开折叠区间）'} onClick={() => setPanelOpen(v => !v)}>
