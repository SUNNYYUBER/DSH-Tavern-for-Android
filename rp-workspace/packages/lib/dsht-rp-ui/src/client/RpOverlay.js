...
function AlternateGreetingsBlock({ ws, openSession, onClose }) {
    ...
    const restart = async (greeting, idx) => {
        ...
        setStatus('新建会话中…');
        try {
            const home = await rpApi('rp/home');
    ...
    return (<div style={{ marginBottom: 12 }}>
      ...
      {greetings.map((g, i) => (<div key={i} className="dsht-rp-altgreet">
          <div className="ag-text">{g}</div>
          <button type="button" className="dsht-rp-btn" style={{ height: 26, fontSize: 12, alignSelf: 'flex-start' }} disabled={busyIdx !== null} onClick={() => { void restart(g, i); }}>
...
function CardDetailDrawer({ ws, openSession, onClose, onSaved }) {
    ...
    const [bound, setBound] = useState(new Set(ws.books.map(b => b.lorePath)));
    const [status, setStatus] = useState('');
    useEffect(() => {
        void rpApi('rp/books')
            .then(r => setLibrary(r.books ?? []))
            .catch(e => setStatus(`书库加载失败：${e.message}`));
    }, []);
    const save = async () => {
        setStatus('保存中…');
        try {
    ...
    return (<div className="dsht-rp-drawer-mask" role="dialog" aria-label={`${ws.name} 详情`}>
      <div className="dsht-rp-drawer">
        ...
        <div className="dsht-rp-drawer-body">
          ...
          {library.map(b => (<label key={b.slug} className="rx-check" style={{ display: 'flex', padding: '7px 0', borderBottom: '1px solid var(--dsw-alias-border-l1)' }}>
              <input type="checkbox" checked={bound.has(b.lorePath)} onChange={() => {
                setBound(prev => {
                    const next = new Set(prev);
                    if (next.has(b.lorePath))
                        next.delete(b.lorePath);
                    else
                        next.add(b.lorePath);
                    return next;
                });
...
function CardExportBlock({ ws }) {
    ...
    const doExportPng = async () => {
        ...
        try {
            const r = await rpApi('rp/export-card', { slug: ws.slug });
    ...
    const doExportBundle = async () => {
        ...
        setStatus('打包卡包目录…');
        try {
            const r = await rpApi('rp/export-bundle', { slug: ws.slug });
            // 逐文件下载（简单可靠；不进 zip 以免依赖）
            for (const f of r.files)
                downloadBase64(f.path, f.binary ? f.content : btoa(unescape(encodeURIComponent(f.content))));
...
export function RpOverlay(props) {
        ...
        try {
            ...
            const merge = (items) => {
                ...
                for (const cfg of configs) {
                    ...
                    if (dsh)
                        merged.push({ ...cfg, name: dsh.title || cfg.name });
                    else
                        missing.push(cfg);
                }
            ...
            const first = await rpApi('workspace-views');
            ...
            if (missing.length > 0) {
                ...
                if (!reconciled && props.refreshSidebar) {
                    try {
                        ...
                        setError(`RP 工作区注册补齐失败：${e.message}——角色仍可在此打开，但侧边栏可能不可见`);
                    }
                }
                if (reconciled) {
                    const second = await rpApi('workspace-views');
    ...
    const launchChar = useCallback(async (ws, fresh) => {
        ...
        try {
            ...
            if (!fresh) {
                ...
                // 卡无开场白则直接复用空白会话。
                if (best && (!best.blank || !ws.firstMes))
     ...
     */
    const refreshAfterImport = useCallback(async (workspaces) => {
        try {
            if (props.refreshSidebar)
                await props.refreshSidebar(workspaces);
        }
        catch { /* 刷新失败不阻塞宫格刷新 */ }
     ...
     */
    const jumpToAdapter = useCallback(async (sessionId) => {
        if (!sessionId)
            return;
        try {
            ...
            setError(`跳转会话失败：${e.message}`);
        }
    }, [props]);
    if (!open)
        return null;
    return (<div className="dsht-rp-overlay" role="dialog" aria-label="DSHTavern 角色扮演">
      ...
      <div className="dsht-rp-main">
        {tab === 'chars'
            ? (<div className="dsht-rp-grid">
              {workspaces.length === 0 && (<div className="dsht-rp-empty">还没有角色。切到「导入」页导入角色卡，或用 SillyTavern 数据整包迁移。</div>)}
              {workspaces.map(ws => (<div key={ws.slug} className="dsht-rp-card-wrap">
                  <button type="button" className="dsht-rp-card" disabled={opening !== null} onClick={() => { void launchChar(ws, false); }}>
                    <div className="name">{ws.name}</div>
                    <div className="meta">{opening === ws.slug ? '打开中…' : `世界书 ${ws.bookCount} 本`}</div>
                  </button>
                  <button type="button" className="dsht-rp-card-gear" style={{ right: 40 }} aria-label={`${ws.name} 新开聊天（带开场白）`} title="新开聊天（带开场白）" disabled={opening !== null} onClick={() => { void launchChar(ws, true); }}>＋</button>
                  <button type="button" className="dsht-rp-card-gear" aria-label={`${ws.name} 详情（世界书绑定）`} title="世界书绑定" onClick={() => { setDetail(ws); }}>⚙</button>
                </div>))}
      ...
      {detail !== null && (<CardDetailDrawer ws={detail} openSession={props.openSession} onClose={() => { setDetail(null); }} onSaved={() => { void loadWorkspaces(); }}/>)}
    </div>);
}
/**
...
function ImportFrame({ onClose, onImported, onKickoff }) {
    useEffect(() => {
        const onMsg = (ev) => {
            ...
            if (t === 'dsht-rp-ui:import-close')
                onClose();
            else if (t === 'dsht-rp-ui:import-done')
                onImported(ev.data.workspaces ?? []);
            else if (t === 'dsht-rp-ui:import-kickoff')
                onKickoff(String(ev.data.sessionId ?? ''));
        };
        window.addEventListener('message', onMsg);
        return () => { window.removeEventListener('message', onMsg); };
    }, [onClose, onImported, onKickoff]);
    return (<iframe title="数据迁移" src="/dsht-rp/import-center" className="dsht-rp-import-frame" sandbox="allow-same-origin allow-scripts allow-forms allow-modals"/>);
}
