 ...
 */
import { planFolds } from './fold-plan.ts';
const columnStates = new WeakMap();
/** turn-tail 行文本里抓「用时 xx / Ran for xx」 */
function scrapeDuration(tailText) {
...
function headerLabel(group, duration, expanded) {
    const parts = [];
    parts.push(duration !== null ? `运行了 ${duration}` : '运行完成');
    if (group.steps > 0)
        parts.push(`${group.steps} 个步骤`);
    if (group.toolCalls > 0)
        parts.push(`${group.toolCalls} 次工具调用`);
    return expanded
        ? `▾ ${parts.join(' · ')}（点击收起过程）`
        : `▸ ${parts.join(' · ')}（点击展开过程）`;
}
function processColumn(column) {
    ...
    if (column.querySelector(':scope > [role="status"]') !== null)
        return;
    ...
    for (const group of plans.values()) {
        ...
        const rowNow = (key) => rowByKey.get(key)?.el?.isConnected === true ? rowByKey.get(key).el
            : column.querySelector(`[data-chat-flow-key="${CSS.escape(key)}"]`);
        ...
        if (!header) {
            ...
            header.addEventListener('click', (e) => {
                ...
                for (const key of foldedKeys) {
                    const row = column.querySelector(`[data-chat-flow-key="${CSS.escape(key)}"]`);
                    if (!row)
                        continue;
                    row.style.display = collapsing ? 'none' : '';
                    if (collapsing)
                        row.dataset.dshtFold = groupId;
                    else
                        delete row.dataset.dshtFold;
                }
                const tailNow = column.querySelector(`[data-chat-flow-key="${CSS.escape(groupId)}"]`);
...
export function installProcessFolder() {
    if (typeof document === 'undefined' || typeof MutationObserver === 'undefined')
        return () => { };
    let timer = null;
    const run = () => {
        timer = null;
        for (const column of Array.from(document.querySelectorAll('[data-chat-flow]'))) {
            try {
                processColumn(column);
            }
            catch (e) {
                console.warn('[dsht-rp-ui] process folder pass failed', e);
            }
        }
    };
    const schedule = () => {
