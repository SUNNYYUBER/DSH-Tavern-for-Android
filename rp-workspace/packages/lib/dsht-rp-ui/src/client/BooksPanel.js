...
export function BooksPanel({ workspaces, onChanged }) {
    ...
    useEffect(() => {
        rpApi('rp/books')
            ...
            .catch(e => setLoadError(`书库加载失败：${e.message}`));
    }, []);
    /** 绑定关系变更：重算该卡的 books 数组 → /rp/bind-books → 失效缓存 + 刷新 */
    const rebind = async (ws, next) => {
        if (busy)
            return;
        setBusy(true);
        setStatus('保存中…');
    ...
    return (<div className="dsht-rp-books">
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 8 }}>
        <button type="button" className="dsht-rp-btn" style={{ height: 30, fontSize: 13 }} onClick={() => { setLoreOpen(true); }}>📖 条目编辑</button>
      </div>
      <p className="dsht-rp-note" style={{ margin: '0 0 12px' }}>
        这里管理「哪张角色卡可以用哪些世界书」。绑定的书会在聊天里按关键词自动触发；改动下一轮对话生效。
      </p>

      {globalBooks.length > 0 && (<div className="dsht-rp-section" style={{ marginBottom: 12 }}>
          <h3>全局生效</h3>
      ...
      {!loadError && library.length === 0 && (<div className="dsht-rp-empty">书库为空——先在「导入」页导入世界书（或整包迁移 ST 数据）。</div>)}

      {library.map(book => {
            const bound = boundCardsOf(book.lorePath);
            const open = expanded === book.lorePath;
            return (<div key={book.slug} className="dsht-rp-section wb-book" data-open={open || undefined}>
            <button type="button" className="wb-row" aria-expanded={open} onClick={() => { setExpanded(open ? null : book.lorePath); }}>
            ...
            {open && (<div className="wb-detail">
                {workspaces.length === 0 && <p className="dsht-rp-note">还没有角色卡——先到「导入」页导入。</p>}
                {bound.length > 0 && (<>
                    <p className="dsht-rp-note">已绑定（点 ✕ 解绑）：</p>
                    <div className="wb-chips">
                      {bound.map(ws => (<button key={ws.slug} type="button" className="wb-chip wb-chip-bound" disabled={busy} aria-label={`解绑 ${ws.name}`} title="点击解绑" onClick={() => { unbind(ws, book.lorePath); }}>{ws.name} ✕</button>))}
                    </div>
                  </>)}
                {unboundCardsOf(book.lorePath).length > 0 && (<>
                    <p className="dsht-rp-note" style={{ marginTop: 8 }}>可绑定（点 ＋ 绑定到该卡）：</p>
                    <div className="wb-chips">
                      {unboundCardsOf(book.lorePath).map(ws => (<button key={ws.slug} type="button" className="wb-chip" disabled={busy} aria-label={`绑定到 ${ws.name}`} title="点击绑定" onClick={() => { bind(ws, book); }}>{ws.name} ＋</button>))}
                    </div>
                  </>)}
              </div>)}
          </div>);
