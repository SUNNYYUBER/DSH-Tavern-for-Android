...
export function MigrationStatusPanel() {
    ...
    if (m) {
        if (typeof m.cards === 'number' && m.cards > 0)
            manifestBits.push(`角色卡 ${m.cards}`);
        if (typeof m.books === 'number' && m.books > 0)
            manifestBits.push(`世界书 ${m.books}`);
        if (typeof m.chats === 'number' && m.chats > 0)
            manifestBits.push(`聊天 ${m.chats}`);
    ...
    return (<div className="dsht-rp-section dsht-rp-verification dsht-rp-fold">
      <div className="dsht-rp-fold-summary-row" role="button" tabIndex={0} onClick={() => { setExpanded(v => !v); }} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ')
        setExpanded(v => !v); }}>
        ...
        <button type="button" className="dsht-rp-back" aria-label="刷新状态" title="刷新" disabled={loading} onClick={(e) => { e.stopPropagation(); void refresh(); }}>{loading ? '…' : '⟳'}</button>
      </div>
      {expanded && (<div className="dsht-rp-fold-body">
        ...
        <div className="dsht-rp-kv">
          ...
          <span className="v">
            ...
            {batch !== null && (batch.report ?? []).map(r => (<div key={r.category}>
        ...
        <div className="dsht-rp-kv">
          ...
          <span className="v">
            {PROBES.map((p, i) => (<div key={p.listName}>
