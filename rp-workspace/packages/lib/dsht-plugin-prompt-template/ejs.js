...
exports.lexTemplate = lexTemplate;
exports.parseTemplate = parseTemplate;
exports.configureEjsCache = configureEjsCache;
exports.protectPreBlocks = protectPreBlocks;
exports.restorePreBlocks = restorePreBlocks;
exports.renderEjsSubset = renderEjsSubset;
exports.isEjsProcessed = isEjsProcessed;
    ...
    while (i < src.length) {
        ...
        if (/[0-9]/.test(c) || (c === '.' && /[0-9]/.test(src[i + 1] ?? ''))) {
            ...
            if (!Number.isFinite(v))
                throw new Error(`bad number: ${src.slice(i, j)}`);
            toks.push({ t: 'num', v });
            i = j;
            continue;
        }
...
function evalExpr(e, scopes) {
    switch (e.k) {
        ...
        case 'bin': {
            if (e.op === '&&')
                return evalExpr(e.a, scopes) && evalExpr(e.b, scopes);
            if (e.op === '||')
                return evalExpr(e.a, scopes) || evalExpr(e.b, scopes);
...
function lexTemplate(template) {
    ...
    const flushText = (end) => {
        if (end > textStart)
            segs.push({ t: 'text', s: template.slice(textStart, end) });
    };
...
function parseTemplate(template) {
    ...
    function parseNodes(inBlock) {
        ...
        while (pos < segs.length) {
            const seg = segs[pos];
            if (seg.t === 'text') {
                nodes.push({ k: 'text', s: seg.s });
                pos++;
                continue;
            }
            if (seg.t === 'out') {
                nodes.push({ k: 'out', expr: parseExpr(seg.expr || 'undefined'), escaped: seg.escaped });
                pos++;
                continue;
            }
            const code = seg.s;
            ...
            if ((m = code.match(RE_ELSE_IF))) {
                if (!inBlock)
                    throw new Error(`'} else if' without matching 'if'`);
            ...
            if (RE_CLOSE.test(code)) {
                if (!inBlock)
                    throw new Error(`'}' without matching block`);
            ...
            if ((m = code.match(RE_IF))) {
                ...
                for (;;) {
                    ...
                    if (sub.term === 'else') {
                        ...
                        if (elseBody.term !== 'close')
                            throw new Error(`'else' block not closed`);
                        branches.push({ cond: null, body: elseBody.nodes });
                        break;
                    }
                    if (sub.term !== 'close')
                        throw new Error(`'if' block not closed`);
                    break;
                }
                pos++; // 吃掉终止代码段
            ...
            if ((m = code.match(RE_FOR))) {
                ...
                const sub = parseNodes(true);
                if (sub.term !== 'close')
                    throw new Error(`'for' block not closed`);
                pos++;
    ...
    if (term !== 'eof')
        throw new Error('template parse error: unbalanced block');
    return nodes;
}
// ---------------------------------------------------------------------------
// 渲染
...
let cacheConfig = { enabled: 0, size: 0, hasher: 'h32ToString' };
