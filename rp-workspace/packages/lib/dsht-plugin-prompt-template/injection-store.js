...
exports.createPromptInjectionStore = createPromptInjectionStore;
...
function createPromptInjectionStore() {
    ...
    return {
        inject(key, prompt, order = 100, sticky = 0, uid = '') {
            ...
            if (normalizedKey === '' || normalizedKey.length > MAX_PROMPT_INJECTION_KEY_CHARS)
                return;
            const normalizedPrompt = injectionText(prompt);
            if (normalizedPrompt.length > MAX_PROMPT_INJECTION_CHARS)
                return;
            ...
            if (entries.length >= MAX_PROMPT_INJECTIONS)
                return;
            entries.push({
                key: normalizedKey,
                prompt: normalizedPrompt,
