...
exports.renderEjsSandbox = renderEjsSandbox;
exports.renderMessagesSandbox = renderMessagesSandbox;
const node_vm_1 = require("node:vm");
const ejs_ts_1 = require("./ejs.ts");
const injection_store_ts_1 = require("./injection-store.ts");
...
function segments(template) {
    ...
    while (cursor < template.length) {
        ...
        if (isMacro) {
            if (text !== '')
                result.push({ kind: 'text', value: literalClosings(text) });
...
function compileTemplate(template, context) {
    ...
    const statements = parsed.map(segment => {
        if (segment.kind === 'text')
            return `__append(${JSON.stringify(segment.value)});`;
        if (segment.kind === 'escaped')
            return `__append(__escape((${segment.value})));`;
        if (segment.kind === 'raw')
            return `__append((${segment.value}));`;
...
function failureKind(error) {
    ...
    if (message.includes('__DSHT_EJS_OUTPUT_LIMIT__'))
        return 'output-limit';
...
function renderEjsSandbox(template, context, options = {}) {
    if (template.length > MAX_TEMPLATE_CHARS)
        return { ok: false, kind: 'source-limit' };
    const code = compileTemplate(template, context);
    if (code === undefined)
        return { ok: false, kind: 'syntax-error' };
    const store = options.injections ?? (0, injection_store_ts_1.createPromptInjectionStore)();
    const sandboxGlobal = {
    ...
    let script;
    try {
        script = new node_vm_1.Script(code, { filename: 'dsht-ejs-template.js' });
    }
    catch {
        return { ok: false, kind: 'syntax-error' };
    }
    const isolated = (0, node_vm_1.createContext)(sandboxGlobal, { name: 'dsht-ejs' });
    try {
...
function renderMessagesSandbox(template, context, messages, options = {}) {
    ...
    for (const msg of messages) {
        if ((0, ejs_ts_1.isEjsProcessed)(msg)) {
            skipped++;
            out.push(msg);
            continue;
        }
