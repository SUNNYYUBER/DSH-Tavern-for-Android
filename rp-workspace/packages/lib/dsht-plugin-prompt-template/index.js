...
exports.name = 'dsht-plugin-prompt-template';
...
exports.inject = ['webServer', 'settings', 'sessions'];
// ---------------------------------------------------------------------------
...
const workerPath = (() => {
    try {
        return (0, node_path_1.join)((0, node_path_1.dirname)((0, node_url_1.fileURLToPath)(import.meta.url)), 'ejs-worker.js');
    }
    catch {
        return null;
    }
...
function renderViaWorker(payload) {
    ...
    return new Promise((resolve) => {
        ...
        try {
            w = new node_worker_threads_1.Worker(workerPath, { workerData: payload });
            const timer = setTimeout(() => done(null), 6_000);
            timer.unref?.();
            w.on('message', (v) => { clearTimeout(timer); done(v); });
...
function apply(ctx, _config) {
    ...
    (0, settings_ns_ts_1.registerSettingsNamespace)(ctx, 'dsht-plugin-prompt-template', 'dsht-ejs');
    (0, http_ts_1.registerPrefix)(ctx, '/dsht-prompt-template', 'dsht-ejs', async (sub, req, res) => {
        ...
        if (sub === '/settings') {
            ...
            const p = (0, node_path_1.join)(dshHome, 'rp', 'ejs-settings.json');
            ...
            if (method === 'PUT' || method === 'POST') {
                ...
                for (const k of Object.keys(defaults)) {
                    ...
                    const v = body[k];
                    const d = defaults[k];
                    if (typeof d === 'boolean' && typeof v === 'boolean')
                        merged[k] = v;
                ...
                if (merged.cacheEnabled !== 0 && merged.cacheEnabled !== 1 && merged.cacheEnabled !== 2)
                    merged.cacheEnabled = 0;
                if (merged.cacheHasher !== 'h32ToString' && merged.cacheHasher !== 'h64ToString')
                    merged.cacheHasher = 'h32ToString';
        ...
        if (sub === '/render') {
            ...
            const cfg = await (0, promises_1.readFile)((0, node_path_1.join)(dshHome, 'rp', 'ejs-settings.json'), 'utf8')
                .then(t => JSON.parse(t)).catch(() => ({}));
            const flag = (k, dflt = true) => cfg[k] === undefined ? dflt : cfg[k] === true;
            ...
            if (!flag('enabled')) {
                if (Array.isArray(body.messages))
                    return (0, http_ts_1.sendJson)(res, 200, { messages: body.messages, rendered: 0, skipped: body.messages.length });
                return (0, http_ts_1.sendJson)(res, 200, { result: String(body.template ?? '') });
            }
            ...
            try {
                if (Array.isArray(body.messages)) {
                    ...
                    if (dbg)
                        console.log(`[dsht-ejs] render messages: depthLimit=${depthLimit} in=${all.length} eligible=${filtered.length}`);
                    let r = null;
                    if (wantWorker) {
                    ...
                    if (!r) {
                        if (engine === 'sandbox') {
                            ...
                            const preList = protectPre ? filtered.map(m => (0, ejs_ts_1.protectPreBlocks)(String(m.mes ?? ''))) : [];
                            const rs = (0, sandbox_ts_1.renderMessagesSandbox)(template, context, preList.length > 0
                                ? filtered.map((m, i) => ({ ...m, mes: preList[i].text }))
                                : filtered);
                            if (rs.ok && preList.length > 0) {
                                rs.messages = rs.messages.map((m, i) => ({ ...m, mes: (0, ejs_ts_1.restorePreBlocks)(String(m.mes ?? ''), preList[i]?.blocks ?? []) }));
                            }
                            r = rs;
                        }
                        else {
                            // 【审查修复 2026-09-05】subset 引擎返回值无 ok 字段——不包装则 !r.ok 恒真 → messages 渲染恒 400
                            r = { ok: true, ...(0, ejs_ts_1.renderMessages)(template, context, filtered, { protectPre }) };
                        }
                    }
                    if (!r.ok)
                        return (0, http_ts_1.sendJson)(res, 400, { error: `[dsht-ejs] ${r.kind}` });
                    const outMsgs = [...all];
                    eligibleIdx.forEach((origIdx, k) => { outMsgs[origIdx] = r.messages[k]; });
                    if (dbg)
                        console.log(`[dsht-ejs] render messages (${engine}${wantWorker ? '+worker' : ''}): rendered=${r.rendered} skipped=${r.skipped}`);
                    return (0, http_ts_1.sendJson)(res, 200, { messages: outMsgs, rendered: r.rendered, skipped: r.skipped + (all.length - eligibleIdx.length) });
                }
                if (!template.trim())
                    return (0, http_ts_1.sendJson)(res, 400, { error: 'template required' });
                if (wantWorker) {
                    ...
                    if (wr) {
                        if (!wr.ok)
                            return (0, http_ts_1.sendJson)(res, 400, { error: `[dsht-ejs] ${wr.kind}` });
                        if (dbg)
                            console.log(`[dsht-ejs] render (sandbox, worker, ${phase}): ${template.length}ch → ${wr.text.length}ch`);
                        return (0, http_ts_1.sendJson)(res, 200, { result: wr.text });
                    }
                    // Worker 不可用 → 落同步
                }
                if (engine === 'sandbox') {
                    const r = (0, sandbox_ts_1.renderEjsSandbox)(template, context);
                    if (!r.ok)
                        return (0, http_ts_1.sendJson)(res, 400, { error: `[dsht-ejs] ${r.kind}` });
                    if (dbg)
                        console.log(`[dsht-ejs] render (sandbox, ${phase}): ${template.length}ch → ${r.text.length}ch`);
                    return (0, http_ts_1.sendJson)(res, 200, { result: r.text });
                }
                const rendered0 = (0, ejs_ts_1.renderEjsSubset)(template, context);
                const result = (0, ejs_ts_1.restorePreBlocks)((0, ejs_ts_1.renderEjsSubset)(protectPre ? (0, ejs_ts_1.protectPreBlocks)(template).text : template, context), protectPre ? (0, ejs_ts_1.protectPreBlocks)(template).blocks : []);
                void rendered0;
                if (dbg)
                    console.log(`[dsht-ejs] render (${phase}): ${template.length}ch → ${result.length}ch`);
                return (0, http_ts_1.sendJson)(res, 200, { result });
            }
            catch (e) {
                return (0, http_ts_1.sendJson)(res, 400, { error: e.message });
            }
        }
        ...
        if (sub === '/check') {
            ...
            return (0, http_ts_1.sendJson)(res, 200, { processed: messages.map(ejs_ts_1.isEjsProcessed) });
        }
        ...
        if (sub === '/permanent') {
            ...
            if (!sessionId || !Number.isInteger(seq) || seq < 0 || !text.trim()) {
                return (0, http_ts_1.sendJson)(res, 400, { error: 'sessionId/seq/text required' });
            }
            const sessions = ctx.sessions;
            const session = sessions?.get?.(sessionId);
            if (!session)
                return (0, http_ts_1.sendJson)(res, 409, { error: 'session not live（B8 永久写回仅支持已打开的会话）' });
            const sAt = session.eventAt;
            const ev = typeof sAt === 'function' ? sAt.call(session, seq) : undefined;
            if (!ev || ev.type !== 'assistant/message')
                return (0, http_ts_1.sendJson)(res, 400, { error: `seq=${seq} 不是 assistant/message` });
            const msg = ev.data?.message;
            if (!msg)
                return (0, http_ts_1.sendJson)(res, 400, { error: '事件缺 message 字段' });
            if (msg.source?.ejsProcessed === true) {
                return (0, http_ts_1.sendJson)(res, 200, { ok: true, note: '已处理（幂等跳过）' });
            }
            const append = session.append;
            if (typeof append !== 'function')
                return (0, http_ts_1.sendJson)(res, 500, { error: 'session.append 不可用' });
            ...
            if (!view.includes(seq))
                return (0, http_ts_1.sendJson)(res, 400, { error: `seq=${seq} 不在当前模型视图（可能已被回退/折叠）` });
            const snap = typeof session.snapshotEvents === 'function'
                ? session.snapshotEvents()
                : [];
            const plan = (0, session_write_ts_1.planAssistantRewrite)(snap, true);
            (0, session_write_ts_1.appendReplace)(session, 'user/message', {
                id: `dsht-ejs-mark-${(0, node_crypto_1.randomUUID)()}`,
                role: 'user',
                content: [{ type: 'text', text: '[EJS 渲染写回] 该楼层原文已从上下文移除，渲染结果随后追加。' }],
                source: (0, session_write_ts_1.markerSource)('dsht-ejs', 'surgical', { renderedFrom: seq, shadowedSeqs: [seq] }),
            ...
            try {
                ...
                return (0, http_ts_1.sendJson)(res, 500, { error: `写回成功但 flush 失败：${e.message}` });
            }
        ...
        if (sub === '/render-entries') {
            ...
            if (!slug)
                return (0, http_ts_1.sendJson)(res, 400, { error: 'slug required' });
            const cfg = await (0, promises_1.readFile)((0, node_path_1.join)(dshHome, 'rp', 'ejs-settings.json'), 'utf8')
                .then(t => JSON.parse(t)).catch(() => ({}));
            if (cfg.enabled === false || cfg.renderLoaderEnabled !== true) {
                return (0, http_ts_1.sendJson)(res, 200, { before: '', after: '', note: 'renderLoader 未启用' });
            }
            const rp = await (0, promises_1.readFile)((0, node_path_1.join)(dshHome, 'rp', slug, 'rp.json'), 'utf8')
                .then(t => JSON.parse(t))
                .catch(() => null);
            if (!rp)
                return (0, http_ts_1.sendJson)(res, 404, { error: `rp.json not found: ${slug}` });
            const entries = [];
            for (const b of rp.books ?? []) {
                if (!b?.lorePath)
                    continue;
                const book = await (0, promises_1.readFile)((0, node_path_1.join)(dshHome, b.lorePath), 'utf8')
                    .then(t => JSON.parse(t))
                    .catch(() => null);
            ...
            const act = (e) => {
                if (cfg.invertEnabled === true)
                    return true; // B18：旧特性兼容——禁用视为启用
                return e.disable !== true;
            };
            const targets = entries.filter(e => /\[\s*RENDER/i.test(e.content) && act(e));
            const vars = sessionId
                ? await (0, promises_1.readFile)((0, node_path_1.join)(dshHome, 'rp', 'state', `${sessionId}.json`), 'utf8')
                    .then(t => JSON.parse(t)).catch(() => ({}))
                : {};
            const context = {
            ...
            return (0, http_ts_1.sendJson)(res, 200, { before: parts.before.filter(Boolean).join('\n'), after: parts.after.filter(Boolean).join('\n') });
        }
