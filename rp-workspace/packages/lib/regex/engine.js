...
exports.runRegexScripts = runRegexScripts;
exports.importRegexScripts = importRegexScripts;
...
function runRegexScripts(scripts, text, timing, placement, ctx = { depth: null }) {
    ...
    for (const script of activeScripts(scripts, timing)) {
        if (!appliesTo(script, placement, ctx.depth))
            continue;
        ...
        if (ctx.substituteMacros)
            replaced = ctx.substituteMacros(replaced);
        if (replaced !== current) {
...
function importRegexScripts(raw) {
    ...
    if (!Array.isArray(raw))
        return { scripts: [], warnings: ['正则数据不是数组'] };
    const scripts = raw.map((item, i) => {
        ...
        return {
            ...
            placement: Array.isArray(r.placement) ? r.placement : [exports.PLACEMENT.AI_OUTPUT],
