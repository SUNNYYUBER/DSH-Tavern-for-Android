...
exports.unwrapMacroWrappers = unwrapMacroWrappers;
exports.extractSkillBlocks = extractSkillBlocks;
exports.renderPresetSkillMd = renderPresetSkillMd;
exports.isAgentPreset = isAgentPreset;
exports.classifyStAgentEntry = classifyStAgentEntry;
exports.detectAgentComposition = detectAgentComposition;
exports.sanitizeSkillName = sanitizeSkillName;
exports.pendingSkillDir = pendingSkillDir;
exports.renderPendingSkillMd = renderPendingSkillMd;
exports.registerStMacros = registerStMacros;
exports.importStPreset = importStPreset;
...
function isGroupHeader(p) {
    ...
    if (HEADER_BRACKET_RE.test(name))
        return true;
    if (HEADER_DECOR_RE.test(name) && content.length <= 200)
        return true;
...
function extractSkillBlocks(presetId, prompts) {
    ...
    for (const p of prompts) {
        ...
        if (!SKILL_REF_PATH_RE.test(body) && !SKILL_HEADER_RE.test(body))
            continue;
...
function isAgentPreset(displayName, prompts) {
    if (AGENT_NAME_RE.test(displayName))
        return true;
...
function classifyStAgentEntry(p) {
    ...
    if (CONFIG_NAME_RE.test(label))
        return 'config';
    if (body.length <= TIER_CONTENT_MAX && CONFIG_CONTENT_RE.test(body))
        return 'config';
    // 档③subagent 编排：名称命中，或同一长文内编排词 ≥2 次
    if (SUBAGENT_NAME_RE.test(label))
        return 'subagent';
    if ((body.match(SUBAGENT_CONTENT_RE)?.length ?? 0) >= SUBAGENT_CONTENT_MIN_HITS)
        return 'subagent';
    // 档②内心 OS/思维链：名称命中，或（短条目）内容命中复合短语
    if (INNER_OS_NAME_RE.test(label))
        return 'innerOs';
    if (body.length <= TIER_CONTENT_MAX && INNER_OS_CONTENT_RE.test(body))
        return 'innerOs';
    return null;
}
/**
...
function detectAgentComposition(displayName, prompts) {
    if (isAgentPreset(displayName, prompts))
        return true;
    for (const p of prompts) {
        const label = `${typeof p.name === 'string' ? p.name : ''}\n${typeof p.identifier === 'string' ? p.identifier : ''}`;
        if (SUBAGENT_NAME_RE.test(label))
...
function registerStMacros(contents) {
    ...
    for (const text of contents) {
        ...
        for (const m of text.matchAll(BARE_MACRO_RE)) {
            const key = m[1];
            if (KNOWN_MACROS.has(key.toLowerCase()) || has(key))
                continue;
            macros[key] = '';
            knownLower.add(key.toLowerCase());
        }
...
function importStPreset(json, displayName) {
    ...
    for (const item of order) {
        ...
        if (usedIds.has(id))
            id = `${id}-${hash36(p.identifier ?? name).slice(0, 4)}`;
        usedIds.add(id);
    ...
    if (typeof o.temperature === 'number')
        sampling.temperature = o.temperature;
    if (typeof o.top_p === 'number')
        sampling.topP = o.top_p;
    if (typeof o.top_k === 'number')
        sampling.topK = o.top_k;
    if (typeof o.frequency_penalty === 'number')
        sampling.frequencyPenalty = o.frequency_penalty;
    if (typeof o.presence_penalty === 'number')
        sampling.presencePenalty = o.presence_penalty;
    if (typeof o.openai_max_tokens === 'number')
        sampling.maxTokens = o.openai_max_tokens;
    if (typeof o.min_p === 'number')
        sampling.minP = o.min_p;
    if (typeof o.top_a === 'number')
        sampling.topA = o.top_a;
    if (typeof o.repetition_penalty === 'number')
        sampling.repetitionPenalty = o.repetition_penalty;
    // ST seed = -1 表示随机（哨兵值，不入登记）
    if (typeof o.seed === 'number' && o.seed >= 0)
        sampling.seed = o.seed;
    if (typeof o.openai_max_context === 'number')
        sampling.maxContext = o.openai_max_context;
    if (typeof o.stream_openai === 'boolean')
        sampling.stream = o.stream_openai;
    ...
    const macros = registerStMacros(prompts.map(p => (typeof p.content === 'string' ? p.content : '')));
    ...
    const preset = {
        ...
        budget: { ...schema_ts_1.DEFAULT_BUDGET },
        sampling,
        ...(macros !== null ? { macros } : {}),
