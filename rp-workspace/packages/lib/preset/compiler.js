...
exports.DSHT_RP_CAPABILITY_GROUP_ID = exports.AGENT_COMPACT_PERSONA_MARKER = void 0;
exports.neutralizePromptVariables = neutralizePromptVariables;
exports.compilePreset = compilePreset;
exports.isDshtRpAgentComposition = isDshtRpAgentComposition;
...
function neutralizePromptVariables(text) {
    ...
    let out = text;
    for (const [find, ph] of PROTECT)
        out = out.split(find).join(ph);
    out = out.split('{{').join('｛｛').split('}}').join('｝｝');
    for (const [find, ph] of PROTECT)
        out = out.split(ph).join(find);
    return out;
}
/**
...
exports.AGENT_COMPACT_PERSONA_MARKER = 'DSHT-RP-COMPACT-PERSONA-V1';
function compilePreset(preset, macroCtx) {
    ...
    const agentRows = preset.path === 'agent'
        ? [
            ...
            `- id: ${exports.DSHT_RP_CAPABILITY_GROUP_ID}`,
            `  name: cordis:group`,
            `  config:`,
            `    - id: skill-filesystem`,
    ...
    const agentYml = [
        ...
        `# ${exports.AGENT_COMPACT_PERSONA_MARKER}`,
        `- id: persona`,
...
exports.DSHT_RP_CAPABILITY_GROUP_ID = 'preset-capability';
/**
