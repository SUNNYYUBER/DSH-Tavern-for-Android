...
exports.planAssistantRewrite = exports.inject = exports.name = exports.findLastUserMessage = exports.truncateSessionJsonl = exports.findStDataRoot = void 0;
...
exports.messageDepth = messageDepth;
...
const session_repair_ts_1 = require("../dsht-plugin-shared/session-repair.ts");
 ...
 */
function applyPromptRegexes(messages, scripts, traceRegexHits, mode = 'persist') {
    if (scripts.length === 0)
        return messages;
    ...
    return messages.map((m, idx) => {
        ...
        const content = m.content.map(block => {
            if (block.type !== 'text' || typeof block.text !== 'string')
                return block;
            const r = (0, engine_ts_1.runRegexScripts)(pool, block.text, 'prompt', placement, { depth });
            if (r.hits.length > 0) {
                changed = true;
...
function collectVariantGroups(events) {
    ...
    for (const ev of events) {
        if (ev.type !== 'assistant/message')
            continue;
        const range = (0, session_write_ts_1.replaceRange)(ev.surfaceOp);
        if (range === null)
            continue;
    ...
    for (const ev of events) {
        ...
        const marker = (0, session_write_ts_1.readMarker)(src, 'surgical');
 ...
 */
var session_write_ts_2 = require("../dsht-plugin-shared/session-write.ts");
Object.defineProperty(exports, "planAssistantRewrite", { enumerable: true, get: function () { return session_write_ts_2.planAssistantRewrite; } });
/**
...
function apply(ctx, _config) {
    ...
    const repairAllSessionSeqs = async () => {
        ...
        for (const h of headers) {
            ...
            try {
                ...
                const v3 = (0, session_repair_ts_1.repairSessionForV3)(norm.content);
                ...
                if (v3.salvaged.length > 0) {
                    ...
                    for (const s of v3.salvaged) {
                        ...
                        for (const [k, val] of Object.entries(s.payload)) {
                            if (k === 'thData')
                                rec.data = val;
                            else if (k === 'thSystem' && val === true)
                                rec.system = true;
                            else
                                rec.legacy = { ...(rec.legacy ?? {}), [k]: val };
                        }
                        table[s.key] = rec;
                    }
                    const n = (0, th_floors_ts_1.mergeSalvagedThFloors)(dshHome, h.sessionId, table);
                ...
                if (v3.changed)
                    console.log(`[dsht-rp] repair-sessions(v3): ${h.sessionId} ${v3.notes.join('；')}`);
                if (r.note)
        ...
        try {
            ...
            if (personaRaw) {
                try {
                    ...
                    if (personaText) {
                        out.push({ name: 'dsht-rp:slot:character', order: tt_projection_ts_1.SLOT_ORDERS.characterCard, text: personaText });
                    }
                }
                catch (e) {
            ...
            try {
                ...
                if (summary)
                    out.push({ name: 'dsht-rp:slot:state', order: tt_projection_ts_1.SLOT_ORDERS.stateTree, text: summary });
            }
            catch (e) {
            ...
            try {
                ...
                if (memoryText) {
                    out.push({
                        ...
                        order: tt_projection_ts_1.SLOT_ORDERS.memory,
            ...
            try {
                ...
                if (active.length > 0) {
                    ...
                    if (tablesText)
                        out.push({ name: 'dsht-memory:slot:tables', order: tt_projection_ts_1.SLOT_ORDERS.tables, text: tablesText });
                }
            }
                        ...
                        if (hits.length > 0) {
                            ...
                            if (changed.length > 0) {
                                out.push({
                                    ...
                                    order: tt_projection_ts_1.SLOT_ORDERS.projectedPrompt,
    ...
    ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
        ...
        try {
            ...
            for (const s of [...selfSections, ...published])
                merged.set(s.name, s);
            if (merged.size > 0) {
                const extra = (0, tt_projection_ts_1.planSlotSections)({ sections: [...merged.values()] }, neutralizeResidualMacros);
                if (extra.length > 0) {
                    assembly = {
                        ...assembly,
    ...
    try {
        if ((0, node_fs_2.existsSync)((0, node_path_1.join)(dshHome, 'rp', 'golden', 'dsht-ENABLED')) && !globalThis.__dshtGoldenFetchPatched) {
            ...
            globalThis.fetch = (async (input, init) => {
                ...
                if (isLlmRoute)
                    console.log(`[dsht-rp] outbound fetch → ${url0.slice(0, 160)}`);
                try {
    ...
    const preparedPromptProjections = new Map();
    /** 【2026-09-10】promptOnly 投影命中记录（诊断用；llm/stream 只读不写，见该钩子头注）。 */
    const projectedPromptHits = new Map();
                    ...
                    projectedPromptHits.set(String(o.sessionId ?? ''), {
                        ...
                        chars: projected.reduce((n, m) => {
                            ...
                            if (typeof c === 'string')
                                return n + c.length;
                            if (Array.isArray(c))
                                return n + c.reduce((k, b) => k + (typeof b.text === 'string' ? (b.text.length) : 0), 0);
                            return n;
                        }, 0),
                    });
    ...
    ctx.on('agent/pre-step', async (raw, next) => {
        ...
        try {
            ...
            let batch = await ctx.waterfall(null, 'dsht-rp/regex', { agent, sessionId: sessionIdForRegex, slug, turn: turnNo, messages: decision.messages, hits: regexHits }, (p) => applyPromptRegexes(p.messages, regexScripts, regexHits, 'persist'));
                ...
                void (async () => {
                    ...
                    try {
                            ...
                            if (live !== undefined && typeof live.append === 'function' && Array.isArray(live.surface?.nodes)) {
                                ...
                                return await withLiveSurgery(sessionId, async () => {
                                    ...
                                    (0, session_write_ts_1.appendReplace)(live, 'user/message', {
                                        ...
                                        role: 'user',
                                        content: [{ type: 'text', text: markerText }],
                                        source: (0, session_write_ts_1.markerSource)('dsht-rp', 'surgical', { regeneratedFrom: anchorEv.seq, shadowedSeqs: seqs }),
                                    }, { start, end }, seqs);
                        ...
                        if (sub === '/variant/switch') {
                            ...
                            const plan = (0, session_write_ts_1.planAssistantRewrite)(eventsForGroups, idle);
                            ...
                            if (plan.openTurn)
                                session.append('turn/start', { turn: plan.turn });
                            ...
                            const shadowedTokens = activeBlocks.reduce((t, b) => t + (b && (b['type'] === 'text' || b['type'] === 'reasoning') && typeof b['text'] === 'string'
                            ...
                            (0, session_write_ts_1.appendReplace)(session, 'user/message', {
                                id: `dsht-variant-mark-${(0, node_crypto_1.randomUUID)()}`,
                                role: 'user',
                                content: [{ type: 'text', text: `[变体切换] 已切换到该楼层的第 ${group.members.findIndex(m => m.seq === targetSeq) + 1}/${group.members.length} 个变体。` }],
                                source: (0, session_write_ts_1.markerSource)('dsht-rp', 'surgical', { variantOf: targetSeq, shadowedSeqs: [group.activeSeq] }),
                            ...
                            if (plan.openTurn)
                                session.append('turn/end', { turn: plan.turn, reason: { kind: 'completed' } });
                        ...
                        if (sub === '/rp/chat/append') {
                            ...
                            const appendMessage = (turn, step, role, text, data) => {
                                ...
                                const mid = `dsht-th-${(0, node_crypto_1.randomUUID)()}`;
                                if (data !== null || role === 'system') {
                                    (0, th_floors_ts_1.upsertThFloors)(dshHome, sessionId, {
                                        [mid]: { ...(data !== null ? { data } : {}), ...(role === 'system' ? { system: true } : {}) },
                                    });
                                }
                                if (role === 'user') {
                        ...
                        if (sub === '/rp/chat/update') {
                            ...
                            for (const t of targets) {
                                ...
                                const oldSidecar = (0, th_floors_ts_1.lookupThFloor)(thFloors, oldMsg.id, seq);
                                ...
                                if (isUser) {
                                    ...
                                    const newId = `dsht-th-${(0, node_crypto_1.randomUUID)()}`;
                                    if (data !== null && typeof data === 'object')
                                        (0, th_floors_ts_1.upsertThFloors)(dshHome, sessionId, { [newId]: { data } });
                                    (0, session_write_ts_1.appendReplace)(live, 'user/message', {
                                        id: newId,
                                        role: 'user',
                                        content: [{ type: 'text', text }],
                                    ...
                                    const markerSourceObj = (0, session_write_ts_1.markerSource)('dsht-tavern-helper', 'surgical', { editedFrom: seq, shadowedSeqs: [seq] });
                                    (0, session_write_ts_1.appendReplace)(live, 'user/message', {
                                        id: `dsht-th-mark-${(0, node_crypto_1.randomUUID)()}`,
                                        role: 'user',
                            ...
                            if (pendingAssistantEdits.length > 0) {
                                const agent = ctx.agents?.get(sessionId);
                                const idle = agent?.phase?.kind === 'idle';
                                const plan = (0, session_write_ts_1.planAssistantRewrite)(sessionEventsSnapshot(live), idle);
                                if (plan.openTurn)
