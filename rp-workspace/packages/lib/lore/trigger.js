...
exports.DEFAULT_TRIGGER_CONFIG = void 0;
exports.visibleMessageCursor = visibleMessageCursor;
exports.isTimedActive = isTimedActive;
exports.buildScanText = buildScanText;
exports.triggerWorldInfo = triggerWorldInfo;
/**
...
exports.DEFAULT_TRIGGER_CONFIG = {
    scanDepth: 2,
    maxRecursionSteps: 5,
    caseSensitive: false,
...
function matchPrimary(entry, text, cfg) {
    for (const key of entry.keys) {
        try {
            if (keyToPattern(key, cfg).test(text))
                return key;
        }
        catch {
            // 无效正则关键词：跳过该关键词
...
function triggerWorldInfo(entries, recentMessages, config = {}) {
    const cfg = { ...exports.DEFAULT_TRIGGER_CONFIG, ...config };
    ...
    for (let round = 1; round <= cfg.maxRecursionSteps; round++) {
        ...
        let newHits = false;
        for (const entry of entries) {
            if (!entry.enabled || activated.has(entry.id))
                continue;
            if (delayBlocked(entry))
                continue;
            if (cooldownActive(entry.id))
                continue; // sticky 生效的第 0 轮已激活，此处只剩 cooldown 抑制
    ...
    const budgetTokens = Math.floor((cfg.budgetPercent / 100) * cfg.contextTokenLimit);
    ...
    const sorted = deduped.sort((a, b) => {
        if (a.entry.constant !== b.entry.constant)
            return a.entry.constant ? -1 : 1;
