...
**用户裁定原文**：「迁移前强制全量备份 + 自动化迁移（推荐）」。
⇒ 关键在「**强制**」二字：它不能是「提醒用户记得点备份」，必须是**代码层面的前置条件**。

**为什么必须在启动路径上拦（而不是加个菜单项）**：

| 事实 | 后果 |
|---|---|
| v3→v4 迁移由**官方运行时在首次打开某会话时自动完成**（我方无法「先预览再决定」） | 「打开老会话」这个动作本身就是**触发点** |
| 迁移**不可逆**（官方明示失败时"拒绝不改源"，成功即原地升版） | 备份必须**早于**这个触发点 |
| 旧版 APK **读不懂** v4 ⇒ 回退到旧版会**丢会话** | 备份是**唯一**退路 |

⇒ 结论：备份动作必须挂在 `MainActivity.onCreate()` 上（用户还来不及点任何会话）。

**实现（`MainActivity.kt`，+213 行）**：

| 件 | 作用 |
|---|---|
| `CURRENT_SESSION_GENERATION = 4` | 代次判定上界；**与单源同源**（见下「漂移判据」） |
| `findLegacySessions()` | 扫描会话目录，只读每个 jsonl 的**第一行**（会话头）取 `version`；`< 4` 即待迁移 |
| `preUpgradeBackup(interactive)` | 停 node（维护模式）→ zip 整个 `$DSH_HOME` → 恢复 node；落 `ExchangeDir.externalSide()` |
| `ensurePreMigrationBackup()` | 启动时调用：无待迁移 ⇒**零开销**短路；已备份过该代次 ⇒ 短路；否则**强制备份** |
| 设置面板新项「升级前安全备份」 | 失败后的**手动重试**入口（不引入自动重试循环） |

**四条关键设计取舍（都写进了代码头注）**：

1. **为什么不复用 `startBackup()`** —— 它走 SAF，**必须用户手选位置** ⇒ **结构上不可能自动跑**。
   ⇒ 自动备份必须落**固定路径**。两者互补：SAF 那个管「换机/归档」，这个管「出事能回退」。
2. **落点 = `ExchangeDir`（交换目录）** —— 用户可见（`/sdcard/Documents/dsht-exchange/`）、
   **不在 App 隐私区、不入库、不进 GitHub**。用户能自己打开看（隐私可控），与 `DiagPack` 同口径。
3. **凭据仍然不进包** —— 复用**同一个** `BACKUP_EXCLUDE`（`.credentials.yaml` / `dsht-token`）。
   备份落在**用户可见**的目录里，更不该带钥匙（恢复后需重填 API Key，与既有口径一致）。
4. **失败 ⇒ 大声报错 + 明确禁令**，不静默降级：
   弹窗写明「检测到 N 个旧格式会话会被**不可逆**升级，而备份没写成 ⇒ **请先别打开这些老会话**」，
   并指向设置面板的重试入口。（**P-30**：静默失败在这里等于「数据无退路」。）

**★ 新增闸门：代次常量不得漂移**（`audit-nodeservice-deploy.mjs` 的**判据 4**）

`CURRENT_SESSION_GENERATION` 若**小于**单源 `dsh-version.json` 的
`sessionFormatKnownGenerations` 最大值 ⇒ 闸门会**漏判**一整代 ⇒ **静默少备份**。
这类缺陷不报错、不崩溃，与①②③同族（静态看全对、只在真机发作、代价是数据）⇒ 必须有判据。

| 控 | 断言 | 结果 |
|---|---|---|
| 正控 F | 常量 = max(knownGenerations) ⇒ 无问题 | ✅ |
| 负控 F | 常量落后一代 ⇒ 报「漂移」 | ✅ |
| 负控 G | 常量缺失 ⇒ 报「未找到」 | ✅ |
| 零控 H | 未提供单源 ⇒ **不误报**（只出声） | ✅ |

**验收**：`audit-nodeservice-deploy.mjs --selftest` **10/10** ·
真实审计 `PASS（4 判据）`（`代次常量一致：CURRENT_SESSION_GENERATION=4 = max(knownGenerations)`）·
`gradle :app:compileReleaseKotlin` **BUILD SUCCESSFUL**。

**★ 本轮自己踩到的两个坑（诚实留痕，都是「报错行远离真凶」型）**：

1. **字符串模板里的 `*/` 会关闭上方的 KDoc 块** —— 我写了
   `"…${bytes / (1024 * 1024)}MB…"`，其中 `*/` 被 Kotlin 词法器读成注释结束符 ⇒
   整文件雪崩式报「Unresolved reference + Unclosed comment」，而报错行在 `preUpgradeBackup`
   **之外**。修法：先算 `val mb = bytes / (1024L * 1024L)` 再插值。
2. **KDoc 里的 `/**` 是「嵌套注释开始」** —— 我在 `ensurePreMigrationBackup` 的文档注释里写了
   `` `$DSH_HOME/**\/*.jsonl` ``，其中的 `/*` 触发嵌套 ⇒ 需要**两个** `*/` 才闭合 ⇒
   同样雪崩。修法：KDoc 里不写 glob 字面量，改用人话（「会话目录下每个 jsonl」）。
...
| C.4 | 新增**锚点指纹**判据（B.2 提议）：命中数相同但指纹变 ⇒ 报「锚点漂移」 | ✅ **已完成**（见下方「C.4 执行记录」） |
...
##### C.4 执行记录（2026-09-23 · **把「命中数相同」与「改的是同一处」拆开**）

**为什么需要它**：已有判据只守「命中数落在期望区间」。但「命中数相同」**不等于**
「改的是同一处」—— 官方完全可能
① 把锚点那行的**语义改了**（换变量名 / 调参数顺序）；② 把它**搬到另一处**
（处数仍为 1，但改的东西完全不同）。这两类漂移**命中数判据一律报 ✓**（假绿），
而后果是补丁打在了「形似而非同一」的地方。

**★ 分工设计（P-40③ 的应用）**：判据必须跑在它所判对象状态**确定之后** ⇒ 拆成两侧：

| 侧 | 落点 | 职责 | 为什么在这里 |
|---|---|---|---|
| **采集** | `apply-platform-patches.py`（构建现场） | 对每处命中原文取 SHA256 前 8 位，落盘 `.dsht-anchor-fingerprints.json` | 它跑的时候**没有「上一代基线」这个概念**；不该让它去猜历史 |
| **比对** | `audit-patch-fingerprints.mjs`（新增） | 读当前 + 上一代基线，同标签指纹**逐元素**比对，不等即报「锚点漂移」 | 它是**独立**判据，缺基线时**跳过并出声**（不冒充通过） |

**指纹口径（两态，都只用官方原文）**：

| 状态 | 取哪段 | 理由 |
|---|---|---|
| **未打补丁** | **锚点 `pattern` 的命中原文** | 最贴近「锚点有没有漂移」的字面口径 |
| **已打补丁** | **marker 之前固定 160 字符窗口** | 锚点已被我方替换掉 ⇒ 采不到原文。该窗口**完全来自官方产物**（我方替换文本只落在 marker 处）⇒ 它变了就是官方那片代码变了。★ 不取 marker **之后**：那之后紧跟的是我方注入的表达式，取它会把**我方模板**的改动误报成漂移 |

**★ 实测踩到的三个坑（全部当场修掉，属「判据自己坏了」族）**：
...
| 判据 | 结果 |
|---|---|
| `audit-patch-fingerprints.mjs --selftest` | **9/9**（正控 + 5 负控 + 2 零控 + 1 缺失控） |
| 真实数据（20 项锚点） | `PASS 锚点指纹无漂移`（20 项一致） |
| ★ **真实负控**（手工把 F2 指纹改成 `deadbeef`） | `✗ 锚点漂移：'F2 Android flock 单进程直通' —— 指纹变了（当前 ["deadbeef"] vs 上一代 ["1c357b3e"]）` ⇒ **精确定位并给出两边指纹** |
| `audit-selftest-claims.mjs` | **136/136 回到全过** |
| 接入 | 已登记进 `build-dsht.ps1` 的 Step 0.5 常驻门禁（含读数行） |
...
| E.5 | vitest：基线 **3~5 failed / 1840~1842 passed**（波动全部来自 `undo.spec.ts` 的既有 git flaky，见 KNOWN-DEBT 第三形态）⇒ 升级后**失败必须全部落在该文件** | ✅ 通过 |

> ★ **E.5 判据口径的一次实测修正（诚实留痕）**：我原先把基线写成「**恰好 4 个**失败」，
> 实测连跑 6 次得到 **3 / 4 / 5 / 4 / 3 / 4** —— **条数本身在抖**。
> ⇒ 若钉死成「恰好 N」，一次正常的 4 失败会被**误报成回归**（假红 ⇒ 训练人忽略报警，**P-38**），
> 而真回归也会**混在这个噪声里**（假绿）。
> ★ **正确判据 = 「失败的必须全部在 `undo.spec.ts` 且根因是 git 子进程」** ——
> 锚到「决定结果的事实」（失败归属 + 根因），而不是锚到一个**本身就在抖动的计数**（**P-45**）。
> 详见 `docs/KNOWN-DEBT-undo-git-flaky-2026-09-23.md` 的「第三形态」。
...
| 步 | 动作 | 状态 |
|---|---|---|
| F.1 | 装 x86_64 APK → node 起来（**0 boot loop**） | ✅ **通过**（+ 出厂包二次复核，见 F.1 附） |
...
##### ★ F.1 附：**出厂 APK 二次复核**（2026-09-24，把「源码对了」与「发出去的包对了」分开验）

**为什么必须单独做这一步**：阶段 F 前半段跑的是**中途构建**的包，且期间我**手工往设备推过
`client.js`**（快速迭代）。那只能证明「源码 + 手工同步后的设备」是对的 ——
**不能**证明「**最终那个 APK 文件**里带的是修好的代码」。
⇒ 故在 arm64 release + x86_64 debug **两包都出炉**之后，**重装出厂 x86_64 包**再验一遍。

**复核结果（全部通过）**：

| 判据 | 观测 |
|---|---|
| 迁移前置闸门仍生效 | logcat `pre-migration gate: 2 legacy session(s), backup already recorded (gen 0)` |
| **「没有落点」警告消失** | logcat 无该行（D.2 修复已进包） |
| RP 侧栏按钮在 | `打开角色扮演（RP）` |
| **已迁移会话可正常打开** | 标题 `【迁移测试】…` · 楼层 `#5 · 9/24, 01:28 AM` · 计数 `3 turns 4 steps` |
| RP 错误呈现链工作 | `本轮运行失败 · MISSING_CREDENTIAL` 卡片（我方 `RpTurnErrorView`） |
| **`设置 → Built-in plugins → DSHTavern` 页签在** | `[data-dsht-plugins-tab]` 计数 **1**，页内四张卡文本齐全 |

★ **结论**：「源码对」「中途构建对」「**出厂包对**」三者**各自独立验证过** ——
这是本次升级交付链上最后一道、也是最容易被跳过的一道判据。
...
**F.4 通过 —— RP 四项逐个实证（4/4）**：

用 WebView CDP（`adb forward` + `webview_devtools_remote_<pid>`）驱动真实 UI 交互，
逐项点开官方前端里我方的 RP 入口：

| # | 功能 | 动作 | 观测结果 | 判定 |
|---|---|---|---|---|
| ① | **状态栏（MVU）** | 点输入区 `🌌` | 「🌌 当前状态」面板打开，含「查看状态 / 搜索 / 状态栏 / 刷新」四件；正确显示「（暂无 MVU 状态——这张卡可能不用变量，或还没产生变量更新）」 | ✅ |
| ② | **世界书** | RP 侧栏点「世界书」 | 「📖 条目编辑」面板打开，文案「这里管理「哪张角色卡可以用哪些世界书」…改动下一轮对话生效」+「书库为空——先在「导入」页导入世界书（或整包迁移 ST 数据）」 | ✅ |
| ③ | **导入** | RP 侧栏点「导入」 | 面板打开并 **实读运行时状态**：`▸ 导入与运行状态：API ✓ · 构建 v388 · 插件 4/4 正常`<br>`▸ 版本与更新：本机 0.2.7 · 运行时 0.1.7-rc.1 · 构建 v388` | ✅ |
| ④ | **回退** | 点楼层 `↩ 回退到此处` | 确认对话框弹出，文案完整：「回退到这条消息？该消息与其后的对话将从上下文移除，原文放回输入框（事件仍保留在日志；状态/变量一并回滚）」+「取消 / 回退」 | ✅ |

★ **③ 是本轮最有分量的旁证**：它不是我写的静态文案，而是 **RP 面板实读运行时**得到的
`运行时 0.1.7-rc.1` + `插件 4/4 正常` + `API ✓` —— 等于**由我方代码自己证明**
「升到 0.1.7 后 4 个插件全部装载成功、API 链路可用」。
...
| 步 | 动作 | 状态 |
|---|---|---|
| G.1 | 双架构 APK（arm64 release + x86_64 debug） | ✅ arm64 release **sentinel v389** · x86_64 debug 同版 |
| G.2 | 发 release（v0.2.7 或 v0.3.0，按破坏性定） | ⏳ **待你拍板版本号**（见下「版本号决策」） |
| G.3 | 文档同步：本审计文档执行记录 / GOAL.md / README / dsh-version.json | ✅ 审计文档 + GOAL.md + `dsh-version.json` 已完成；README 待发版时同步 |

> **版本号决策（为什么我停在 0.2.7 没有自作主张跳 0.3.0）**：
> 本次升级**对用户可见行为是零变化的**（同一套 API、同一套 UI；迁移是官方自动的），
> 但**底层换了数据格式代次**（v3 → v4）。按 SemVer 惯例这属于「minor 级别的内部变更」——
> 可算 0.2.7 的补丁式重发，也可算 0.3.0 的「数据模型换代」。
> ★ 用户已裁定过「**先 0.1.5-rc.3 发一版，再单独攻 0.1.7**」—— 即本次**就是**那个「单独攻 0.1.7」，
> 故**建议仍发 0.2.7**（保持版本单调，把 0.3.0 留给真正的功能换代）；
> 但这条**属发布策略、不是技术判断** ⇒ 我不擅自决定。

##### G 交付前置清单（**逐条可核对，全部有证据**）

| # | 项 | 证据 | 状态 |
|---|---|---|---|
| 1 | 版本单源 = `0.1.7-rc.1` | `rp-workspace/dsh-version.json`（含 BOM） | ✅ |
| 2 | runtime **267 子包零漂移** | `audit-dsh-version.mjs` 判据 ⑤ | ✅ |
| 3 | **13 条补丁全命中** | `apply-platform-patches.py` + **锚点指纹 20 项无漂移** | ✅ |
| 4 | 六段验收 **6 OK / 0 BLOCK / 0 UNKNOWN** | `audit-upgrade-readiness.mjs` | ✅ |
| 5 | vitest 失败**全部**落在 `undo.spec.ts` | 连跑 6 次实测（3~5 波动，见 E.5） | ✅ |
| 6 | 真机：**0 boot loop** + **v0→v4 迁移** + RP 四项 | 上方 F 执行记录 | ✅ |
| 7 | **迁移前置备份闸门**真机生效 | `dsht-prebak-*.zip` 落交换目录、凭据 **0 条** | ✅ |
| 8 | 双架构 APK 内嵌 runtime 同版 | `verify-apk-runtime-version.mjs` | ✅ |
| 9 | **M4 内容级 169 项 / 缺失 0** | `verify-apk-payload.py` | ✅ |
| 10 | ★ **回滚路径可用**（迁移前备份 = 唯一退路） | §E.6 + 第 7 项 | ✅ |
