...
| **③** | W-3/W-4 交付（W-5 按可行性如实收口） | ✅ | `文件`：W-3 = [DeviceBridge.kt](file:///d:/DSH%20RolePlay/rp-workspace/android/app/src/main/java/com/dshtavern/app/DeviceBridge.kt)（仅 127.0.0.1 + token fail-closed；**唯一命令构造点**，`List<String>` 数组不经 shell ⇒ 结构上免疫注入）+ [dsht-plugin-device](file:///d:/DSH%20RolePlay/rp-workspace/packages/src/dsht-plugin-device/index.ts) 4 个工具 + 18 条测试全绿；W-4 = [Gate.kt](file:///d:/DSH%20RolePlay/rp-workspace/android/app/src/main/java/com/dshtavern/app/Gate.kt) danger 档挂起 + **默认拒绝** + 单次有效 + 三态入审计日志，`audit-device-tiers.mjs` selftest 6/6。W-5 如实收口 = sharp 官方 prebuild 无 android 但**主包自带 wasm 兜底** ⇒ 接 `@img/sharp-wasm32`，spike **19/19 PASS**，并把原 stub 的**静默空实现**收紧为受控报错。`命令`：`audit-plugin-build-parity.mjs`（selftest 6/6）+ `audit-nodeservice-deploy.mjs`（selftest 6/6）本次构建实跑通过；**三条真机 boot loop 事故（部署面漏包 / 顶层 inject 不可得 / `patchReload: live`）全部修复并真机终局验证**（`attempt 1` 零重启） |
...
| **⑤** | 全套 vitest 绿 + 构建门禁绿 + CI 出包实证 + 发新版 APK | ✅ | `命令`（本次实跑）：① 引擎回归 **86 文件 / 1835 通过 / 2 skipped / 0 失败**（`npm test --prefix packages`）；② 构建门禁 **53 条 `[gate] OK`、退出码 0**（`build-dsht.ps1 -DshVersion 0.1.5-rc.1 -SkipInstall -Arch x86_64`），含 `audit-artifact-freshness.mjs`（产物与现场重编译**逐字节一致**）、`verify-apk-runtime-version.mjs`、`verify-apk-payload.py`（M4 内容级标记齐全，缺失 0）；③ CI：v0.2.4 tag 上 `build-apk` **`success`**（17m31s）；`发布`：v0.2.4 为 **Latest、非 draft**，双架构资产实读在场 |
| **⑥** | 真机/鸿蒙验证按既定口径众包，不阻塞上述收口 | ✅ | 本条件为**豁免条款**，其自身成立即可。应用记录：②的 `getUid()==2000`、W-3 执行层 UserService 真机接入、鸿蒙真机 —— 均登记为众包回填项（见 B-DEVICE 清单），**未阻塞 ①③④⑤ 的收口** |
