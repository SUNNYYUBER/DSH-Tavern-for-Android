...
| **⑤** | 全套 vitest 绿 + 构建门禁绿 + CI 出包实证 + 发新版 APK | ✅ | `命令`（终局实跑）：① 引擎回归 **86 文件 / 1839 通过 / 2 skipped / 0 失败**；② 构建门禁 **55 条 `[gate] OK`、退出码 0**（含 `audit-native-deps.mjs` / `audit-device-honesty.mjs` 两条本轮新增），另有 `audit-publish-hygiene.mjs` **0 项**；③ CI：v0.2.6 tag 上 **全绿**（`Fetch native libs` ⇒ `Build x86_64` ⇒ `Attach to release`）；④ `发布`：**v0.2.6 为 Latest、非 draft、非 prerelease**，双架构资产实读在场（arm64 145,950,979 B + x86_64 146,739,195 B）；⑤ **用 CI 产出的那个文件**在模拟器实证：boot loop **0**、无 `CANNOT LINK`、web 起在 3080、runtime/lib 含 **17** 个库 |
| **⑥** | 真机/鸿蒙验证按既定口径众包，不阻塞上述收口 | ✅ | 本条件为**豁免条款**，其自身成立即可。应用记录：②的 `getUid()==2000`、③的 W-3 四工具真机端到端实证、arm64 真机与鸿蒙真机 —— 均登记为众包回填项，**未阻塞 ①③④⑤ 的收口** |

**终局结论（逐条）**：①✅ ②⚠️（第三项按第⑥条众包） ③✅（真机实证按第⑥条众包） ④✅ ⑤✅ ⑥✅（豁免条款自身成立）。

**目标达成判定**：本 goal 的**可执行部分已全部完成** —— 全部工作面（W-1~W-11）交付或按既定口径如实收口，
最新版 APK 已编译发布（**v0.2.6**，双架构、Latest、CI 全绿），并以**用户实际下载的产物**完成模拟器端到端实证。
剩余仅第 ② 条的 `getUid()==2000` 与 arm64/鸿蒙真机实测 —— 这两项由**完成定义第 ⑥ 条**明确豁免为众包，
**不构成 goal 未完成**。

**本轮最重要的一条方法论教训（在两个版本上连续验证了两次）**：
- **v0.2.4 那次**：本表把第 ③ 条判为 ✅，而证据并不支持 —— `exec()` 实为恒返
  `NOT_IMPLEMENTED`，`out/` 无任何实跑记录。当时之所以「看起来完成」，
  是因为一切都**静态全绿**（编译过、单测过、门禁过、真机 boot 正常、HTTP 桥在监听）。
- **v0.2.5 那次（更严重）**：在**已发布的产物**上发现**双架构 node 完全无法启动**
  （`libz.so.1 not found`）—— 根因是**注释把「预期」写成了「事实」**，
  而编译期、单测、全部既有门禁**一律绿灯**；用户拿到的是「装得上但核心功能为零」的包。
- ⇒ **教训：在本项目里「绿灯」与「可用」是两件事，不能以绿灯推断完成**（P-30 家族：
  代理量与事实脱钩）。
- ⇒ **对策不是「下次更仔细」，而是把这两族缺陷做成构建期报红的常驻门禁**：
  `audit-device-honesty.mjs`（守「能力声明 vs 实际接线」诚实，selftest 13/13）与
  `audit-native-deps.mjs`（守「ELF 依赖闭合」，selftest 8/8）。
  后者已在 CI 上抓到**第二例**（proot 依赖库只写在注释里）——
  证明它不是一次性补丁，而是真的在持续工作。
