...
1. **harness 层转为防守**：维护现有平台补丁（成本已被门禁机器化压到很低）+
...
4. **狂飙突进（2026-09-20 晚追加，用户裁定）**：项目定位 = **思路分析驱动**；
   实机验证众包给社区回填，理论/工具/文档工作永不等实机。单人 vs 团队的赢面在
   **速度与知识资产密度**（冲突类型学、门禁体系、验证方法论都是别人没有的复利）。
   三个追加方向：**插件生态基础设施**（dsh-plugin-lint 守门人）、**MVU 生态深耕**、
   **补丁上游化**（平台无关补丁提 PR 回 DSH 官方，降维护成本 + 话语权）。
...
### 正式版门槛（2026-09-20 用户拍板：两步走 + 鸿蒙不阻塞 + 实机测试众包）

> **战略口径修正（2026-09-20 晚，用户原话要点）**：本项目更多是**思路分析**性质；
> 实机测试交给感兴趣的开发者和用户（众包回填），理论工作**不得因等待实机而停滞**。
> 单人对抗团队型竞品（DSHA）的策略 = **狂飙突进**。

当前 v0.2.0-beta.5（pre-release）。**v0.2.0 正式版**门槛（修正后）：

- [ ] **npm 首发闭环**：`npm login` 后 `node scripts/publish-plugins.mjs --publish`（脚本与门禁已就绪）
- [ ] **口径诚实**：所有「已验证」标注到验证面（模拟器/真机/鸿蒙分列），未实测面如实标注——
  这本来就是项目纪律，正式版只是把它显式化
- ~~真机闭环 / 稳定期~~ → **降级为社区众包项**：B-DEVICE 清单保持开放，
  欢迎开发者/用户回填（小米/卓易通占位行保留）；不再阻塞发版节奏

鸿蒙（卓易通）**不阻塞**正式版，release notes 如实标「未实测」；测后回填 B0 回归矩阵。
后续 **v1.0.0** 另立门槛（届时再评：MVU 生态成熟度 / 插件市场 / DSH 0.1.6 契约稳定）。
