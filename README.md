...
| 不是 | 说明 |
|---|---|
| ❌ **不是 SillyTavern 的移植版** | 没有一行 ST 源码，只读它的资产格式 |
| ❌ **不是 100% 兼容** | ST 自己发新版都会弄坏一批卡。我们给分级承诺（见下） |
| ❌ **不是云端服务** | 全部本地运行；API Key 只存在本机 |
| ❌ **不含任何内容** | 没有角色卡 / 世界书 / 对话，全部由你自己导入 |
